...
<script type="text/javascript">
	...
	function onMouseWheel(e){
		...
		if(e.wheelDeltaY > 0 && getScrollTop()==0){
			if(parent.yys>1){
				shouldScroll = false
				...
				parent.changePage(parseInt(parent.yys)-1);
				setTimeout(function(){
					var scroll_top = document.body.scrollHeight - getWindowHeight()
					document.documentElement.scrollTop = scroll_top;  
					window.pageYOffset = scroll_top;  
					document.body.scrollTop = scroll_top; 
				},2)
				
				//$(document.body).scrollTop(document.body.scrollHeight - getWindowHeight())
				
			} 
		} else if(e.wheelDeltaY < 0 && (document.body.scrollHeight-getScrollTop()<=getWindowHeight())){
			if(parent.yys<imageCount){
				shouldScroll = false
				window.onmousewheel = null;
				setTimeout(function(){
					window.onmousewheel=onMouseWheel;
	...
	function changeScroll(){
		...
		if((document.body.scrollHeight-getScrollTop()<=getWindowHeight())){
			if(parent.yys<imageCount){
			...
			if(getScrollTop()==0){
				if(parent.yys>1){
					window.onscroll = null;
					parent.changePage(parent.yys(parent.yys)-1);
					document.body.scrollTop = document.body.scrollHeight - getWindowHeight()
					setTimeout(function(){
						window.onscroll=function(){
							 changeScroll();
...
<div style="margin:0 auto;" >
  <div  style="display: none;" id="jzk2">
    ...
    <div style="display: inline;background-color:#989898;z-index:8;padding-left: 16px;" id="OBJECTbox">
    	<OBJECT id="OCXpdfobj" codebase="" style="width:1024px;height:800px;margin-left:-18px;" classid="clsid:ECCC5C8C-8DA0-4FAC-935A-CD5229A14BCC" ></OBJECT>
    </div>
</div>
</div>
