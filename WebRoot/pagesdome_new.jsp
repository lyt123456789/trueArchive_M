...
<html>
  	<head>
    	...
    	<script src="${ctx}/widgets/component/taglib/attachment/js/attachment.js?t=201902011833" type="text/javascript"></script>
    	...
    	<script type="text/javascript">
	    	...
	    	function reloadOcx(attId,isFq){//重新加载控件将正文修改内容重新加载
	    		/*if(!isFq){
	    			isFq = false;
	    		}*/
	    		if(attId){
	    			...
	    			setTimeout(function(){
		    			$.ajax({
		    				type:'post',
		    				url:'${ctx}/attachment_reloadAttById.do',
		    				data:{
		    					"attId":attId/*,
		    				...
		    				error:function(){
		    					alert('AJAX调用错误(attachment_reloadAttById.do)');
		    				},
		    				success:function(data){
		    					...
		    					//}else{
		    						attObj = eval("("+data+")");
		    						SetReplaceFileInfo(attObj.id,attObj.pdfUrl,attObj.pageCount);
		    					//}
				    			attId="";
		    				}
		    			});
  	...
  	<body onload="formsub();" style="overflow:hidden;background-color:#989898">
    	...
    	<div class="wrap" style="margin:0; height: 36px;">
      		...
      		<div class="header">
				...
				<div class="cg_tit">
      			...
      			<h1></h1>
      			<div class="navmen">
		        	<span style="overflow:hidden;white-space:nowrap;-o-text-overflow:ellipsis;text-overflow:ellipsis;line-height: 40px;">流程图</span>
		        </div>
		        ...
		        <div class="zw1" style="right:138px;width:122px;" onclick="PrintCurPage();"><img src="${ctx}/assets-common/newImage2/dy.png"><a href="javascript:void(0)" style="line-height: 30px">打印当前页</a></div>
		        
		        <div class="zw1" style="right: 106px;width: 24px;color: white;cursor: pointer;" onclick="changeBigger();" title="放大">
		        	<i class="iconfont icon-fangda" style="padding: 0 4px 0 4px;position: relative;top: 0px;" title="放大"></i>
		        </div>
		        <div class="zw1" style="right: 74px;width: 24px;color: white;cursor: pointer;" onclick="changeLitter();" title="放大">
		        	<i class="iconfont icon-suoxiao" style="padding: 0 4px 0 4px;position: relative;top: 0px;" title="放大"></i>
		        </div>
      		...
      		<div class="main clearfix">
        		<div class="cg_con fl" style="width: 1024px;margin: 0 auto;z-index: 30;left:0;right:0;top:0;position:fixed;background-color:#F8F8F8;">
          			<div class="cg_info jQtabcontent" style="margin-top:46px">
            			<div class="fj_load fj_load_2" style="overflow-y: hidden;border: none;">
              				<c:if test="${itemType eq '0' }">
								...
								<c:forEach items="${zwAtts}" var="zwAtt">
	              					<div class="file-item">
		                				...
		                				<c:if test="${zwAtt.filetype ne 'ceb'&&zwAtt.filetype ne 'pdf'&&zwAtt.filetype ne 'PDF'&&zwAtt.filetype ne 'CEB' }">
		               					<span style="vertical-align:-6px;" id="${zwAtt.id}" ><img src="${ctx}/assets-common/newImage2/xz.png" onclick="download('${zwAtt.localation}','${zwAtt.id} ','${zwAtt.filename}','${downloadUrl}?name=${zwAtt.filename}&location=${zwAtt.localation}',downloadCb);"></span>
		               					</c:if>
		               					...
		               					<c:if test="${zwAtt.filetype eq 'doc' || zwAtt.filetype eq 'docx'}"><!-- 是否是word  -->
		               						...
		               						<span style="vertical-align:-6px;"><img src="${ctx}/assets-common/newImage2/history.png" onclick="viewHistory('${docguid}attzw','${zwAtt.id}');"></span>
		               					</c:if>
	               					</div>
	              				</c:forEach>
	              				</div>
	              				<c:if test="${zwTemSel eq 'true'}">
	              				<div class="c_btn btn-type-2" style="margin:5px;display:none;width: 86px;height: 24px;" onclick="openUploadTemplate('','${docguid}attzw','${isFirst}','true','false','true','false','false','','','','','','false','${nodeId}','${isFirst}','${serverurl}');">
	              					...
	              					<div class="file-item">
		                                ...
		                                <c:if test="${fjAtt.filetype ne 'ceb'&&fjAtt.filetype ne 'pdf'&&fjAtt.filetype ne 'PDF'&&fjAtt.filetype ne 'CEB' }">
		               					...
		               					<c:if test="${allowUpload eq 'true'}">
		               						<c:if test="${isFirst==true}">
			               						<c:if test="${fjAtt.filetype eq 'doc' || fjAtt.filetype eq 'docx'}">
			               							<span style="vertical-align:-6px;"><img src="${ctx}/assets-common/newImage2/watch.png" onclick="viewDoc('${fjAtt.localation}');"></span>
			               						</c:if>
			               						<c:if test="${fjAtt.filetype eq 'pdf' || fjAtt.filetype eq 'PDF'}">
			               							<span style="vertical-align:-6px;"><img src="${ctx}/assets-common/newImage2/watch.png" onclick="viewPDF('${fjAtt.localation}','${fjAtt.filename}');"></span>
			               						</c:if>
		               						</c:if>
	              					...
	              					<div class="file-item">
		                				...
		                				<c:if test="${fjAtt.filetype ne 'ceb'&&fjAtt.filetype ne 'pdf'&&fjAtt.filetype ne 'PDF'&&fjAtt.filetype ne 'CEB'}">
		               					<span style="vertical-align:-6px;" id=${fjAtt.id} >
		               						<img src="${ctx}/assets-common/newImage2/xz.png" onclick="download('${fjAtt.localation}','${fjAtt.id}','${fjAtt.filename}','${downloadUrl}?name=${fjAtt.filename}&location=${fjAtt.localation}',downloadCb);">
		               					</span>
		               					</c:if>
		                				<c:if test="${fjAtt.filetype eq 'ceb' or fjAtt.filetype eq 'pdf' or fjAtt.filetype eq 'PDF' or fjAtt.filetype eq 'CEB'}">
			                				<c:if test="${fn:contains(fjAtt.editer,userId)||fgwDownload}">
      		...
      		<div class="nav fl" style="z-index:40;position:absolute;width:100px;margin-top:40px;">
       		...
       		<div class="nav2 fl" style="margin-top:40px;position:absolute;z-index:40;right:0;min-height: 800px;">
				<div class="list">
 						<ul class="yiji">
   						<li id="toNextStep" >
   							<a style="width: 100%;text-align: left;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;" href="#" class="inactive">下一步</a>
      						<ul style="display: none" id="sendnext">
      						<c:if test="${isSendAgain != null && isSendAgain eq 1 && empty needNewInput}">
								<div>
      						...
      						<c:if test="${send !='' && send !=0}">
	      						<c:forEach items="${nodes_last}" var="nodelast">  
									<c:if test="${nodelast.wfn_name!=null && nodelast.isBigNum == '0'}">
										<div>
											<li id="${nodelast.wfn_id}" onclick="routeType('${nodelast.wfn_id}','node','','','${nodelast.wfl_child_merge}','${nodelast.wfn_route_type}')"><a href="#" title="${nodelast.wfn_name}">${nodelast.wfn_name}</a></li>
		 								</div>
		 							</c:if>
 								</c:forEach>
 								<c:forEach items="${nodes}" var="node">  
						    		<c:if test="${node.wfn_name!=null && node.isBigNum != '0'}">
										<div>
											<li id="${node.wfn_id}" onclick="routeType('${node.wfn_id}','node','','','${node.wfl_child_merge}','${node.wfn_route_type}')"><a href="#" title="${node.wfn_name}">${node.wfn_name}</a></li>
						    			</div>
						    		</c:if>
					    		</c:forEach>
					    		...
					    		<c:forEach items="${childs}" var="child">  
									<c:if test="${child.wfc_cname!=null && child.wfc_cname!=''}">
										<div>
											<li id="${child.wfc_cid}" onclick="routeType('${child.wfc_cid}','childWf','${child.wfc_relation}','${child.wfc_ctype}','','')"><a href="#" title="${child.wfc_cname}">${child.wfc_cname}</a></li>
						    			</div>
						    		</c:if>
					    		</c:forEach>
					    		<!-- 多流程节点条件选择 -->
					    		<c:if test="${nodeName!=null && nodeName!='' }" >
					    			<li onclick="choiceCondition('${nodeId}');" id="flowNode" ><a href="#" title="${nodeName}">${nodeName}</a></li>
					    		</c:if>
				    		</c:if>
				    		<c:if test="${send==0 and empty needNewInput}">
								<c:if test="${showEndBtn != 'true'}">
									<div>
										<li onclick="operateForm(1,'');"><a href="#" title="办理完成">办理完成</a></li>
									</div>
								</c:if>
								<c:if test="${showEndBtn == 'true'}">
									<div>
										<li onclick="end('end');"><a href="#">办理完成</a></li>
									</div>
								</c:if>
							</c:if>
							<c:if test="${not empty needNewInput and needNewInput=='true'}">
								<div>
								<c:choose>
									<c:when test="${not empty notWriteForm and notWriteForm=='true' }">
										<li onclick="overNotWriteForm();"><a href="#">发送</a></li>
									</c:when>
									<c:otherwise>
										<li onclick="over();"><a href="#">发送</a></li>
									</c:otherwise>
								</c:choose>
								</div>
							</c:if>
							<c:if test="${self_loop=='1'}">
								<div>
									<li id="${self_node.wfn_id}" onclick="routeType('${self_node.wfn_id}','node','','','${self_node.wfl_child_merge}','${nodelast.wfn_route_type}','${self_node.wfn_self_loop}')"><a href="#" title="${self_node.wfn_name}">${self_node.wfn_name}</a></li>
								</div>
							</c:if>
							...
							<c:if test="${isSend == true && isEndProcess == true && send !=0 && isNeedGwjh == true}">
								<div >
				     				<li id='gwjh' onclick="send()"><a href="#">公文交换</a></li>
								</div>
							</c:if>
   						...
   						<li>
   							<a href="#" class="inactive qitacaozuo">其他操作</a>
     							<ul style="display: none">
	      							<c:if test="${itemId!=null}">
	       							...
	       							<li onclick="downloadPdf('${processId}');"><i class="iconfont iconfont-41download"></i><a style="min-width:83px" href="#">下载</a></li>
	       							<c:if test="${favourite != '1'}">
        								<li onclick="javascript:addCollection();"><i class="iconfont iconfont-starton"></i><a style="min-width:83px" href="#" id="collect">收藏</a></li>
        							</c:if>
	       							<c:if test="${favourite == '1'}">
        								<li onclick="javascript:addCollection();"><i class="iconfont iconfont-starton"></i><a style="min-width:83px" href="#" id="collected">取消收藏</a></li>
        							</c:if>
									...
									<li onclick="javascript:SearchText();"><i class="iconfont iconfont-save"></i><a style="min-width:83px" href="#">检索</a></li>
	                               	<c:if test="${!needHideOperate}">
		                               	<li onclick3="setDrawType(2,0,0,0,3);" onclick="onOptionClick(this, 5)">
		                               		<i class="iconfont icon-bi"></i>
		                               		<a style="min-width:83px" href="#" title="画笔"><span>画笔</span></a>
		                               	</li>
										<!-- <li title="" onclick3="setDrawType(3,0,0,0,3);" onclick="onOptionClick(this, 6)">
											<i class="iconfont icon-zhixian"></i>
											<a style="min-width:83px" href="#" title="画线"><span>画线</span></a>
										</li>
										<li title="箭头" onclick3="setDrawType(6,0,0,0,3);" onclick="onOptionClick(this, 4)">
											<i class="iconfont icon-jiantou-copy-copy"></i>
											<a style="min-width:83px" href="#" title="箭头"><span>箭头</span></a>
										</li>
		                               	<li title="曲线箭头" onclick3="setDrawType(7,0,0,0,3);" onclick="onOptionClick(this, 3)">
		                               		<i class="iconfont icon-quxianjiantou"></i>
		                               		<a style="min-width:83px;padding-right: 0;" href="#" title="曲线箭头"><span>曲线箭头</span></a>
		                               	</li>
										<li title="波浪线" onclick3="setDrawType(4,0,0,0,3);" onclick="onOptionClick(this, 9)">
											<i class="iconfont icon-bolangxian"></i>
											<a style="min-width:83px" href="#" title="波浪线"><span>波浪线</span></a>
										</li> -->
									</c:if>
                                   	<!-- <li title="放大">
   							...
   							<li onclick="showHandWrite(1);" style="background-color:#008cee;border-radius: 5px;">
   								<a href="#"  title="签批人信息" style="padding: 0;text-align: center;width: 100%;"><span>签批人信息</span></a>
   							</li>
 						</ul>
				</div>
				<div id="toTop" style="position: fixed;width: 33px;height: 58px;z-index: 2;right: 40px;bottom: 100px;" onmouseover="toTopMouseOn();" onmouseout="toTopMouseOut();">
    	...
    	<script type="text/javascript">
	    	...
	    	function onColorPicked(isRed){
	    		if(isRed){
	    			$('.pen-color-red').addClass('selected-pen');
	    			$('.pen-color-black').removeClass('selected-pen');
	    			__color = 255;
	    		} else {
	    			$('.pen-color-red').removeClass('selected-pen');
	    			$('.pen-color-black').addClass('selected-pen');
	    			__color = 0;
	    		}	
	    		if(__optionIndex == 5){
	    			setDrawType(1,__color,0,0,3)
	    		} else if(__optionIndex == 6){
	    			setDrawType(2,__color,0,0,3)
	    		} else if(__optionIndex == 4){
	    			setDrawType(6,__color,0,0,3)
	    		} else if(__optionIndex == 3){
	    			setDrawType(7,__color,0,0,3)
	    		} else if(__optionIndex == 2){
	    			setDrawType(5,__color,0,0,3)
	    		} else if(__optionIndex == 1){
	    			setDrawType(4,__color,0,0,3)
	    		}else if(optionIndex == 7){
	    			setDrawType(10,__color,0,0,3)
	    		} else if(optionIndex == 8){
	    			setDrawType(9,__color,0,0,3)
	    		} else if(optionIndex == 9){
	    			setDrawType(11,__color,0,0,3)
	    		}
	    	...
	    	function onOptionClick(_this, optionIndex){
	    		debugger;
	    		for(var i=1;i<10;i++){
	    			if(i == optionIndex){
	    		...
	    		//$('.sub-toolbar-container ul').hide();
	    		
	    		__optionIndex = optionIndex;
	    		
	    		if(optionIndex == 5){
	    			addSelectHandle(_this);
	    			if(ifLaKuang == false && preIndex == optionIndex){//判断拉框签批状态，设置相应绘画功能参数
	    				ifLaKuang =  true;
	    				setDrawType(0,__color,0,0,3)//拉框签批功能
	    			}else{
	    				ifLaKuang =  false;
	    				setDrawType(1,__color,0,0,3)//画笔功能
	    			}
	    		} else if(optionIndex == 6 ){
	    			addSelectHandle(_this);
	    			if(ifLaKuang == false  && preIndex == optionIndex){//判断拉框签批状态，设置相应绘画功能参数
	    				ifLaKuang =  true;
	    				setDrawType(0,__color,0,0,3)
	    			}else{
	    				ifLaKuang =  false;
	    				setDrawType(2,__color,0,0,3)
	    			}
  				...
  				$('.header .fj').click(function(){
					...
					var c_btn_height = $(".btn-type-1").height()
  					...
  					if($('.header .fj').hasClass('btn-selected')){
  						...
  						$('.fj_file').css({'display':'none'});
  						$('.c_btn').css({'display':'none'});
  						document.getElementById("frame1").contentWindow.setPageSize(size[3] + 36, 1024);
  					} else {
  						...
  						$('.fj_file').css({'display':'block'});
       					...
       					document.getElementById("frame1").contentWindow.setPageSize(size[3] + 15 - fjHeight - c_btn_height, 1024);
  					}
  				});
  				$('.header .zw').click(function(){
  					var fjHeight = $(".file-type-2").height()
					var c_btn_height = $(".btn-type-2").height()
					var fj_load_2 = $(".fj_load_2").height()
  					...
  					if($('.header .zw').hasClass('btn-selected')){
  						...
  						$('.fj_file').css({'display':'none'});
  						$('.c_btn').css({'display':'none'});
  						document.getElementById("frame1").contentWindow.setPageSize(size[3] + fjHeight, 1024);
  					} else {
		...
		<script>
			...
			function reissue(){
				...
				$.ajax({   
					...
					success : function(nodeInfo) { 
						var msg = nodeInfo.split(",")[0];
						var exchange = nodeInfo.split(",")[1];   //给发送下一步传所点击的按钮的属性值--下一步的节点值
						var url4UserChose='${curl}/ztree_showDepartmentTree.do?siteId=${siteId}&showCheckbox=1&exchange='+exchange+'&click=true&nodeId=${nodeId}&send=${send}&nopeople=1&routType='+msg;
						var url = url4UserChose;
						var WinWidth = 1000;
						var WinHeight = 620;
						SetIsOpenDlg(1);
					...
					$.ajax({
						...
						success : function(data) {
							if(data=="0"){
								...
								/* //var srcFile ='${ctx}/table_downloadPdfOnlyFj.do?processId='+processId;
								var url4UserChose='${ctx}/table_downloadPdfOnlyFj.do?processId='+processId;
								var url = url4UserChose;
								var WinWidth = 300;
								var WinHeight = 200;
								if(url4UserChose!=null && url4UserChose.indexOf("table_downloadError.do")){
									 if(top.window && top.window.process && top.window.process.type){
										console.info("封装打开方式");
									    var ipc = top.window.nodeRequire('ipc');
									    var remote = top.window.nodeRequire('remote');
			...
			function RefreshView() {
				try{
					document.getElementById("frame1").contentWindow.RefreshView();
				}catch (e) {
				}
			}
			function GetNextUserID() {
				try{
					return document.getElementById("frame1").contentWindow.GetNextUserID();
				}catch (e) {
				}
			}
			function GetAdviceJsonData(str) {
				try{
					return document.getElementById("frame1").contentWindow.GetAdviceJsonData(str);
				}catch (e) {
				}
			}
			function GetCurAdviceJsonData() {
				try{
					return document.getElementById("frame1").contentWindow.GetCurAdviceJsonData();
				}catch (e) {
				}
			}
			function SetReplaceFileInfo(szFileID,szFileUrl,iFileCount) {
				try{
					return document.getElementById("frame1").contentWindow.SetReplaceFileInfo(szFileID,szFileUrl,iFileCount);
				}catch (e) {
				}
			}
			function Preview(json){
				try{
					return document.getElementById("frame1").contentWindow.Preview(json);
				}catch (e) {
				}
			}
			...
			function over(){
				var obj = document.getElementById("frame1").contentWindow;
				var oldComment = GetCurAdviceJsonData();
				var canSendMsg = '';
				var userIds = GetNextUserID();
				userIds = userIds.split("$$")[0];
				document.getElementById("toName").value = userIds.split('@')[0];
				document.getElementById("sendMsgUserId").value = userIds.split('@')[1];
				if(document.getElementById("sendMsgUserId").value){
					canSendMsg = 1;
				}else{
					canSendMsg = 0;
				}
				if(userIds){
					...
					setTimeout(function(){
						doReissue(userIds,canSendMsg);
					}, 500);
				}else{
					if(stringNotEmpty(oldComment)){
						if('${showEndBtn}'!='true'){
							operateForm(1,'')
						}else{
							end('end');
				...
				/* var nodeId = '${nodeId}';
				var url4UserChose='${curl}/table_pleaseOrWatch.do?siteId=${siteId}&nodeId='+nodeId+'&oldComment='+encodeURI(oldComment)+'&t='+new Date();
				var url = url4UserChose;
				var WinWidth = 700;
				var WinHeight = 250;
				SetIsOpenDlg(1);
				if(top.window && top.window.process && top.window.process.type){
				    ...
				    ipc.on('message-departmentTree',function(args){
						if(treeWin){
							...
							treeWin.close();
							treeWin = null;
							if(stringNotEmpty(args)){
								var retId = args.split("||")[0]; 
								var retName = args.split("||")[1]; 
								if(!stringNotEmpty(retId)){
									var comment = GetAdviceJsonData(retName);
									...
									if(stringNotEmpty(comment)){
										...
										var canSendMsg = '';
										var sendMsgId = '';
										if(retId.split("#").length>1){
											canSendMsg = retId.split("#")[1];
											rets = retId.split("#")[0];
											if(rets.split("@").length>1){
												ret = rets.split("@")[0];
												sendMsgId = rets.split("@")[1];
