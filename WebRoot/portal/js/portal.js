...
function initTopListEvent(){	
	$.ajax({
		...
		dataType:"jsonp",
		jsonp:"callback",
		url:portalurl+"portalInterface_getModuleMenuJson.do?isadd=1&type=1&userId="+userId,
		cache: false,
		async: false,
	    success:function(msg){
	    	//$(".JS_eventList").empty();
...
function initSystemInfo(){	//业务系统初始化
	$.ajax({
	    ...
	    success:function(msg){
			...
			var html = "";
			for(i=0;i<length;i++){
				if(sysList[i].isInnerLink == "0" || sysList[i].isInnerLink == "1"){
					if(sysList[i].isInnerLink == "1"){
						html += "<li id='"+ sysList[i].id +"'> <i class='tp-sys-icon'> <img src='"+ sysList[i].icon +"'> </i> <a class='tp-sys-name' maxlink='"+ sysList[i].link +"' onclick='maxMod(this)' target='_blank'>"+ sysList[i].title +"</a><div class='tp-mod-close' data-id='"+ sysList[i].id +"'><span class='tp-icon-del'></span></div></li>";
					}else{
						html += "<li id='"+ sysList[i].id +"'> <i class='tp-sys-icon'> <img src='"+ sysList[i].icon +"'> </i> <a class='tp-sys-name' href='"+ sysList[i].link +"' target='_blank'>"+ sysList[i].title +"</a><div class='tp-mod-close' data-id='"+ sysList[i].id +"'><span class='tp-icon-del'></span></div></li>";
					}
			    }else{
			...
			$('.tp-mod-close').click(function(){
				var that = this;
				var data = {};
				data.id = $(this).attr("data-id");
	 			$.ajax({
					url:"${ctx}/portalMenu_delPrivateLinks.do",
					type:"post",
					async:true,
					cache: false,
					data:data,
					success:function(result){
						layer.tips("取消成功",that,{
							tips:[1,"#0fa6d8"]
						});
						initSystemInfo();
	...
	if( kind == "dynamic" ){
		...
		$.ajax({
		    ...
		    success:function(msg){
		    	...
		    	if(contents==""){
					$(contentBody).html("<div class='tp-msg'><img src='/trueOA/assets-common/image/noneData.png' /></div>");
			...
			contentBody.find("a").each(function(){
				...
				if(openType!=='0' && openType!==0){
					...
					$(this).off("click").on("click",function(e){
						...
						var id = GetQueryString('messageId',link) != '' ? GetQueryString('messageId',link) : GetQueryString('id',link) ;
						var title = $(this).attr('title');
						e.preventDefault();
						topOpenLayerTabs(id,1200,600,title,link+'&t='+new Date());
					    ...
					    success:function(msg){
					    	...
					    	if( msg.success == false){
						    	...
						    	if(contents==""){
									...
									tabPanel.html("<div class='tp-panel-cnt'><div class='tp-msg-a'><img src='/trueOA/assets-common/image/noneData.png' /></div></div>");
								}else{
									if(tabType == "event"){				
