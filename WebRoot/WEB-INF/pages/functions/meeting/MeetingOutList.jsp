...
<div class="pageHeader" >
	<form action="${ctx }/meeting_getMeetingOutList.do" method="post">
	<div class="searchBar">
...
<form id="thisForm" method="POST" name="thisForm" action="${ctx }/meeting_getMeetingOutList.do" >
	<div id="w_list_print">
	<table class="list" width="100%" targetType="navTab" asc="asc" desc="desc" layoutH="116">
		<thead>
			<tr>
				...
				<th width="20%" align="center">通知标题</th>
				<th width="20%" align="center">会议时间</th>
				<th width="20%" align="center">会议地点</th>
				
				<th width="10%" align="center" title="操作">操作</th>
			</tr>
		</thead>	
		<tbody>
		<c:forEach var="item" items="${meetingOutList}" varStatus="status"> 
			<tr >
				<!-- <td align="center">
				...
				<td align="center"><c:if test="${item.status == '0'}"><font color="red">办理中</font></c:if><c:if test="${item.status == '1'}"><font color="red">已办结</font></c:if></td>
				 --%>
				 <td align="left" title="${item.meetingname }"><span style="width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><a href="#" onclick="openForm('${item.wf_process_uid}','${item.item_id}','${item.form_id}');" >${item.meetingname }</a></span></td>
				 <td align="left" title="${item.newtitle }"><span style="width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${item.newtitle }</span></td>
				 <td align="center" title="${item.newtime }"><span style="width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${item.newtime }</span></td>
				 <td align="left" title="${item.arr }"><span style="width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${item.arr }</span></td>
				 <td align="center">
					<a href="javascript:chmd('${ item.wf_instance_uid }');" >参会名单</a>
				</td>
			</tr>
			</c:forEach>
		</tbody>
...
<script>
 ...
 window.onload=function(){ 
		//获得后台分页相关参数-必须
		skipUrl="<%=request.getContextPath()%>"+"/meeting_getMeetingOutList.do";			//提交的url,必须修改!!!*******************
