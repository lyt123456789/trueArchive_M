...
<body>
 	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="edit" href="javascript:xg_row();" ><span>填报名单</span></a></li>
			<li class="line">line</li>
		</ul>
	</div>
<div class="pageContent">
	
<form id="chrylist" method="POST" name="chrylist" action="${ctx }/meeting_chrylist.do" >
	<div id="w_list_print">
	<table class="list" width="100%" targetType="navTab" asc="asc" desc="desc" layoutH="116">
		...
		<c:forEach var="item" items="${list}" varStatus="status"> 
			<tr >
				...
				<td align="center"><a href="#" onclick="openForm('${item.wf_process_uid}','${item.item_id}','${item.form_id}');">${item.meetingname }</a></td>
...
<script>
 ...
 window.onload=function(){ 
		//获得后台分页相关参数-必须
		skipUrl="<%=request.getContextPath()%>"+"/meeting_chrylist.do";				//提交的url,必须修改!!!*******************
		submitForm=document.getElementById('chrylist');								//提交的表单,必须修改!!!*******************
	...
	for(var i = 0 ; i < selid.length; i++){
	 ...
	 if(ids.length > 0){
	 	...
	 	if(ids.indexOf(',')>-1){
	 		...
	 		location.href = "${ctx}/meeting_toEditJsp.do?id="+ids+"&type=1";
	 	}
	 }else{
		alert('请选择要填报名单的对象！');
    }
