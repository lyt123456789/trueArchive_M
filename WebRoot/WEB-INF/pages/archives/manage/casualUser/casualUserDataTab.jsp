...
<html>
	...
	function getTabPageData(idchild) {
		...
		var idchild = idchild;
		var obj = {
			"casualId":"${casualId}",//临时用户ID
        	"treeNode":structureId,//节点ID
        	"dataIdChild":idchild
		};
		$.ajax({
			url:"${ctx}/role_getCasualUserDataRange.do",
			type:"post",
			async:false,
			cache: false,
			dataType:"json",//返回值类型
	...
	function deleteCard(dom) {
		...
		if(confirm('确定删除所选权限吗')){
			...
			$.ajax({
		        ...
		        dataType:"json",//返回值类型
		        data:{"id":id},
		        url:"${ctx}/role_delCausualUserDataRange.do",
		        success:function(data){
	...
	function editCard(dom) {
		...
		var condition = "?casualId=${casualId}&structureId=${structureId}&aeFlag=edit&idchild="+dataIdChild+"&title="+title+"&id="+id;
		layer.open({
		    type: 2,
		    title: "设置数据查询条件",
		    shade: 0.4,
		    area: ['970px', '380px'],
		    content: "${ctx}" + "/role_toSetCasualUserDataRange.do"+condition,
		    cancel: function(){
		        //右上角关闭回调
		   	 	window.location.reload();
		   }
	...
	function setTabDataRange(dom) {
		...
		$.ajax({
			url:"${ctx}/role_checkCasualUserAllotData.do",
			type:"post",
			async:false,
			cache: false,
			dataType:"json",//返回值类型
			...
			success:function(data){
				var flag = data.have;
				if("have" == flag) {
					alert("抱歉，您已设定该临时用户的查询权限，请点击下方编辑按钮进行编辑或删除后再添加！");
				} else {
					var title = encodeURI(encodeURI($("#title").val()));
					var condition = "?casualId=${casualId}&structureId=${structureId}&aeFlag=add&idchild="+idchild+"&title="+title;
					layer.open({
        			    type: 2,
        			    title: "设置数据查询条件",
        			    shade: 0.4,
