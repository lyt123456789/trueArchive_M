...
<div class="wf-layout">
	<div class="wf-list-top">
		<div class="wf-search-bar">
			<form name="lendLingList"  id="lendLingList" action="${ctx }/btnmanage_toBtnManagePage.do?a=Math.random();" method="post" style="display:inline-block;width: 100%;">
				<div class="search">
	...
	<div class="wf-list-wrap">
		...
		<table id="tab232323" class="wf-fixtable" layoutH="140"  style="width:100%;">
			<thead>
				<tr>
	    	  		<th width="2%"></th>
	    	  		<th align="center" width="14%">按钮名称</th>
	    	  		<th align="center" width="14%">按钮描述</th>
	    	  		<th align="center" width="12%">排序号</th>
                   </tr>
	    	</thead>
	    	<c:forEach items="${list}" var="data" varStatus="status">
	    		<tr>
					...
					<td align="center"  title ="${data.btn_name}" >${data.btn_name }</td>
					<td align="center"  title ="${data.btn_description}" >${data.btn_description}</td>
					<td align="center"  title ="${data.btn_sort}" >${data.btn_sort}</td>
	    		</tr>
	    	</c:forEach>
		</table>
	</div>
	...
	window.onload=function(){ 
		//获得后台分页相关参数-必须
		skipUrl="<%=request.getContextPath()%>"+"/btnmanage_toBtnManagePage.do";			//提交的url,必须修改!!!*******************
	...
	$("#addTemplate").bind("click",function(){
		layer.open({
			...
			area:['500px','510px'],
			closeBtn:0,
			content:"${ctx}/btnmanage_toBtnManageAddPage.do?"
		});
	});
	...
	function del(){
		...
		if(confirm('确定删除所选数据吗')){
			$.ajax({
				url:"${ctx}/btnmanage_checkBtnManageDelete.do",
				type:"post",
				async:false,
				cache: false,
				dataType:"json",
				...
				success:function(data){
					...
					if(sfv=="delete"){
						...
						$.ajax({
					        ...
					        dataType:"json",
					        data:{"ids":ids},
					        url:"${ctx}/btnmanage_deleteBtnManage.do",
					        success:function(data){
					        	 var sfv = data.flag;
									if(sfv=="yes"){
										window.location.reload();
	...
	function update(){
		...
		if(ids==""){
		 ...
		 openPending("修改按钮信息","/btnmanage_toBtnManageEditPage.do?id="+ids);
	}
