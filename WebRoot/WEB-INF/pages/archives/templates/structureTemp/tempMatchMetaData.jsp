					...
					<thead>
	                 ...
	                 <c:forEach items="${list}" var="item" varStatus="status">
                    	<tr>
                    		...
                    		<td align="center" title ="${item.tagName}">${item.tagName}</td>
                    		<td align="center" title ="${item.tagType}">${item.tagType}</td>
                    		<td align="center" axis="0" id="${item.id}" title="${item.id_MetaData }">${item.metaDataFullName}</td>
                    	</tr>
                    </c:forEach>
				</table>
			</div>
...
<script type="text/javascript">
	...
	**/
	...
	function saveAllMetaData() {
		...
		$.ajax({
			url:"${ctx}/str_saveTagsMetaData.do",
			type:"post",
			async:false,
			cache: false,
			dataType:"json",
			...
			success:function(data){
				...
				if(retList.length == 0) {
					...
					for(var c = 0; c < retList.length; c++) {
						var esmt = retList[c];
						$("#"+esmt.id).empty();
						$("#"+esmt.id).append("<div style='background:#aee6df'>"+ esmt.metaDataFullName +"</div>");
						$("#"+esmt.id).attr("title",esmt.id_MetaData);
						$("#"+esmt.id).attr("axis","1");
					}
