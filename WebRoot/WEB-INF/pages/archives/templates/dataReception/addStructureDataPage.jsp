				...
				<c:if test="${tag.esLength<200}">
					...
					<c:choose>
					   <%--  <c:when test="${map_DMZ[key_DMZ] ne null}">
					    ...
					    <c:otherwise>
					        <input type="text" id="C${tag.id}" name="C${tag.id}" value="<c:if test="${not empty map_MRZ[key_MRZ]}">${map_MRZ[key_MRZ]}</c:if>"
							<c:if test="${tag.esIsNotNull eq '1'}">style="background-color: #ffeeee;border: 1px solid #ff7870;"</c:if>
...
function checkData(){
	...
	for(var i =0;i<tags.length;i++){
		var obj = document.getElementById("C"+tags[i]);
		if(obj&&obj.value==""){
			flag=false;
			obj.focus();
			top.layer.msg("存在必填数据未填写！");
			break;
		}
	}
	return flag;
