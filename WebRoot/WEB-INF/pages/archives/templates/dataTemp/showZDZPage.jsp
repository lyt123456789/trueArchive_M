...
<html>
	...
	<body>
		...
		<div id="isDisplayRight">
			...
			<div id="computeAddField" class="commonFather">
				<div class="title">选择自动累加字段的参照字段</div>
				<div id="" class="marLef">
					<div class="layui-transfer layui-form layui-border-box" lay-filter="LAY-transfer-1">
						<div class="layui-transfer-box" data-index="0" style="width: 200px; height: 360px;">
							<div class="layui-transfer-header">
							...
							<div class="layui-transfer-search">
								<i class="layui-icon layui-icon-search"></i>
								<input type="input" class="layui-input" onkeyup="searchLike('computeAddField','left')" placeholder="关键词搜索">
							</div>
							<ul class="layui-transfer-data" style="height: 267px;">
						...
						<div class="layui-transfer-box" data-index="1" style="width: 200px; height: 360px;">
							...
							<div class="layui-transfer-search">
								<i class="layui-icon layui-icon-search"></i>
								<input type="input" class="layui-input" placeholder="关键词搜索" onkeyup="searchLike('computeAddField','right')">
							</div>
							<ul class="layui-transfer-data" style="height: 267px;">
							</ul>
						</div>
				...
				<div class="buttonDiv">
					<button type="button" title="置顶" class="layui-btn layui-btnUpDownmax upMaxBtn" onclick="moveUpMax('computeAddField')"> 
						<i class="layui-icon">&#xe619;</i>
						<i class="layui-icon">&#xe619;</i>
					</button>
					<button type="button" title="上移" class="layui-btn layui-btnUpDown upBtn" onclick="moveUp('computeAddField')"> 
						<i class="layui-icon">&#xe619;</i>
					</button>
					<button type="button" title="左移" class="layui-btn layui-btnUpDown rightBtn" onclick="moveToLeft('computeAddField')">
						<i class="layui-icon layui-icon-prev"></i>
					</button>
					<button type="button" title="右移" class="layui-btn layui-btnUpDown leftBtn" onclick="moveToRight('computeAddField')">
						<i class="layui-icon layui-icon-next"></i>
					</button>
					<button type="button" title="下移" class="layui-btn layui-btnUpDown downBtn" onclick="moveDown('computeAddField')">
						<i class="layui-icon">&#xe61a;</i>
					</button>
					<button type="button" title="置底" class="layui-btn layui-btnUpDownmax downMaxBtn" onclick="moveDownMax('computeAddField')">
						<i class="layui-icon">&#xe61a;</i>
						<i class="layui-icon">&#xe61a;</i>
					</button>
				</div>
	...
	<script type="text/javascript">
		...
		function showMoreAddData(dom) {
			if(!$(dom).hasClass("layui-form-checked")) {//没有选中
				...
				$.ajax({
					url:"${ctx}/dataTemp_getZDZAppendFields.do",
					type:"post",
					async:false,
					cache: false,
					dataType:"json",
					...
					success:function(data){
						...
						var yuanData = twoData.yuanData;
						var id_tag = twoData.id_tag;
						$("#checkIdTag").val(id_tag);
						var leftUl = $("#computeAddField").find("ul").eq(0);
						var rightUl = $("#computeAddField").find("ul").eq(1);
						//设置源数据
						if(yuanData == null || typeof(yuanData) == 'undefined' || yuanData[0] == null || typeof(yuanData[0]) == 'undefined') {
							$(leftUl).empty();
						...
						var targetData = twoData.targetData;
						//设置目标数据
						if(targetData == null || typeof(targetData) == 'undefined' || targetData[0] == null || typeof(targetData[0]) == 'undefined') {
							$(rightUl).empty();
		...
		function saveChooseData(dom) {
			...
			for(var i = 0; i < li.length; i++) {
				...
				if(idTag == checkIdTag) {
					obj["tagIds"] = tagIds;
					if(tagIds.length > 0) {
						obj["deleteFlag"] = "update";//指的是选中的自动累加字段的参照字段存在并要求修改
					} else {
						obj["deleteFlag"] = "delete";//指的是选中的自动累加字段的参照字段不存在要删除
					}
				} else {
					obj["tagIds"] = "";
					obj["deleteFlag"] = "noChange";//指的是<未选中>ps：*重要*的参照字段无须修改
				}
				obj["idTag"] = idTag;
			...
			if(computeArr.length <= 0) {
				layer.confirm('确定删除自动累加字段？', {icon: 3, title:'提示'}, function(index){
					saveZDXZData(saveObj);
				  	layer.close(index);
				}); 
			} else {
				if(isEmpty(tagIds)) {
					layer.confirm('确定删除自动累加字段的参照字段？', {icon: 3, title:'提示'}, function(index){
						saveZDXZData(saveObj);
					  	layer.close(index);
					}); 
				} else {
		...
		function saveZDXZData(obj) {
			$.ajax({
				url:"${ctx}/dataTemp_saveofZDZData.do",
				type:"post",
				async:true,
				cache: false,
				dataType:"json",
