...
<body>
	<form name="form1" id="form1" method="post" enctype="multipart/form-data" action="${ctx }app/zxwdController/addAnswer">
		<input id="wordays" type="hidden" name="wordays" value="${str}"/>
		<input id="type" type="hidden" name="type" value="${type }"/>
		<!-- 
	<div class="pagescon">
		<p class="p5">
			<span class="toparr">当前位置：</span>
			<span class="hscol">日历更新</span>
		</p>
	</div> -->
					...
					str+="<tr>"; 
						for(var k=1;k<=7;k++){
							...
							if(index>=weekday&&index1<=monthday){
								value=index1;
								//if(k==1||k==7){
								//	str+="<td class=\"noworkday\" title=\"点击设置工作日\" onclick=\"setNoworkday(this)\">"+value+"</td>";//默认选中星期天和星期六为节假日
								//}else{
								//	str+="<td class=\"workday\" title=\"点击设置工作日\" onclick=\"setNoworkday(this)\">"+value+"</td>";
								//};
								if(workday!=''){
									var workdays=workday.split(/,/);
									var cyear=tyear;
									var cmonth=i<10?"0"+i:""+i;
									var cday=index1<10?"0"+index1:""+index1;
									var ctoday=cyear+'-'+cmonth+'-'+cday;
									var isin=false;
									//alert(ctoday);alert(workdays);
									for(var h=0;h<workdays.length;h++){
										if(ctoday==workdays[h]){
											isin=true;break;
										}
									};
									if(isin){
										...
										str+="<td class=\"\" style=\"background-color: #F9CC76;cursor: pointer;\" title=\"点击设置工作日\" onclick=\"setNoworkday(this)\">"+value+"</td>";
									}
					...
					str+="<tr>"; 
						for(var k=1;k<=7;k++){
							...
							if(index>=weekday&&index1<=monthday){
								value=index1;
								if(k==1||k==7){
									str+="<td class=\"\" title=\"点击设置工作日\" onclick=\"setNoworkday(this)\">"+value+"</td>";//默认选中星期天和星期六为节假日
								}else{
									str+="<td class=\"\" title=\"点击设置工作日\" onclick=\"setNoworkday(this)\">"+value+"</td>";
								};
								index1++;
							}else{
								str+="<td>"+value+"</td>";
