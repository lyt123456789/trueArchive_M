...
<html>
    ...
    <body>
        <div class="bread-box"><div class="bread-title"><span class="bread-icon"></span><span class="bread-links"><span class="bread-start">当前位置：</span><span class="bread">公文管理 </span><span class="bread-split">&raquo;</span><span class="bread">已发公文(${deptName})</span></span><span class="bread-right-border"></span></div></div>
             ...
             <form id="searchForm" action="${ctx}/send_sentDocList.do?type=${type}&departmentIds=${departmentIds}" method="post">
             <div class="displayTableForm">
            	<table class="fr" cellspacing="0" cellpadding="0">
            		<tr>
            			<td class="vm">
							<span style="font-size: 14; font-family: 黑体;">&nbsp;&nbsp;部门:</span>
						</td>
						<td class="vm">
							<select mice-select="select" id="departmentIds" name="departmentIds">
								<c:forEach items="${deps}" var="dep">
									<option value="${dep.departmentGuid}" <c:if test="${dep.departmentGuid eq departmentIds}">selected="selected"</c:if>>${dep.departmentName}</option>
								</c:forEach>
                ...
                <display:table name="requestScope.sendedList" class="displayTable" pagesize="10" htmlId="displayTable" id="element" partialList="true" export="false" size="requestScope.size" excludedParams="*" requestURI="send_sentDocList.do" form="searchForm">
      ...
      <script type="text/javascript">
      		...
      		document.onkeydown = keyDown;
   			...
   			seajs.use('lib/form',function(){
   				...
   				$('#departmentIds').cssSelect({trigger:true,triggerFn:function(){
					window.location.href="${ctx}/send_sentDocList.do?type=${type}&departmentIds="+$(this).val();
				}});
