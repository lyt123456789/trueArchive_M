...
%>
<div class="bread-title"><span class="bread-icon"></span><span class="bread-links"><span class="bread-start">当前位置：</span><span class="bread">公文管理 </span><span class="bread-split">&raquo;</span><span class="bread">已收公文(${deptName})</span></span><span class="bread-right-border"></span></div></div>

<form id="chaxunform" name="chaxunform" method="post" action="${ctx}/rec_receivedDocList.do?type=${type}&departmentId=${departmentId}&listType=show">
<div class="displayTableForm">
	<table class="searchUL">
	<tr>
	<td width="25">部门</td>
	<td><select mice-select="select" id="departmentId" name="departmentId" class="departmentId">
	...
	<td width="45">来文号</td>
	<td><input type="text" mice-input="input" id="wh" name="wh" value="" style="width:120px;"/></td>
	<td width="25">标题</td>
	<td><input value="" type="text" mice-input="input" id="title" name="title" style="width:120px;"/></td>
	<td width="50">来文单位</td>
	<td><input value="" type="text" mice-input="input" size="12" id="sendername" name="sendername" style="width:120px;"/></td>
	</tr>
	</table>
...
<display:table class="displayTable" name="requestScope.docList" 
	...
	requestURI="rec_receivedDocList.do?listType=show" form="chaxunform">
	...
	<display:column headerClass="split" class="docTitle" title="标题"sortable="false"><label id="${element.docguid}title" value="${element.title}"><a href="javascript:if('${departmentId}'==='') window.location.href='${ctx}/rec_receivedDocDetail.do?id=${element.docguid}&deptId=${element.queueXto}&listType=show'; else window.location.href='${ctx}/rec_receivedDocDetail.do?id=${element.docguid}&deptId=${departmentId}&listType=show';" target="_self"><c:if test="${element.status==0}"><span style="color: #1C488E">${element.title}&nbsp;</span></c:if><c:if test="${element.status==1}"><span style="color: #3B5F00">${element.title}&nbsp;</span>	</c:if><c:if test="${element.status==2 or element.status==3 or element.status==ygl}">${element.title}&nbsp;</c:if><c:if	test="${element.status==4}"><span style="color: #C90F1F">${element.title}&nbsp;</span></c:if></a></label></display:column>
...
<script type="text/javascript">
	...
	seajs.use('lib/form',function(){
		...
		$('.departmentId').cssSelect({trigger:true,triggerFn:function(){
			window.location.href="${ctx}/rec_receivedDocList.do?type=webid&listType=show&departmentId="+$(this).val();
		}});
		...
		if (window.confirm('确认撤销!')) {
			$.ajax( {
				...
				success : function(msg) {
					if (msg == 'success') {
						alert("成功撤销！");
						window.location.href="${ctx}/rec_receivedDocList.do?type=webid&departmentId="+$('#departmentId').val();
	...
	$("#exportRecDoc").bind('click',function(){
		var isPass=true;
		$.ajax( {
			url : '${ctx}/rec_checkExportRownum.do',
			type : 'POST',
			cache : false,
			async : false,
			data : {
