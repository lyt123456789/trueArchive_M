...
<body>
	<table class="displayTable">
		...
		<c:forEach items="${groups}" var="group">
			<tr><td class="docTitle"><a href="#" onclick="javascript:getGroup('${group.dep}');">${group.name}</a></td><td class="docType"><a href="javascript:deleteGroup('${group.id}');"><span title="点击删除该组" style="display:inline-block;width:16px;height:16px;cursor:pointer;background:url(${ctx}/widgets/theme/dm/images/icon_main.gif);background-position: 0 -20px;"></span></a></td></tr>
		</c:forEach>
	</table>
	<a id="flush" href="#"></a>
	...
	<script type="text/javascript">
		function deleteGroup(id){
			...
			document.getElementById('flush').href='${ctx}/selectTree_showGroup.do';
			document.getElementById('flush').click();
		}
		function getGroup(dep){
			window.returnValue = dep;
			window.close();
		}
