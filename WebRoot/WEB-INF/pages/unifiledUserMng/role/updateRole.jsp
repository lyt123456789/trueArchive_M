	...
	<div class="tw-layout">
		<div class="tw-container">
		    <form method="get" id="roleForm" name="roleForm" action="#" class="tw-form">
		    	<input id="roleId" name="roleId" value="${id }" type="hidden"></input>
		    	...
		    	<input id="deptIdAndNames" name="deptIdAndNames" value="${deptIdAndNames }" type="hidden"></input>
		    	<input id="portalRes" name="portalRes" value="${portalRes }" type="hidden"></input>
		    	<input id="oaRes" name="oaRes" value="${oaRes }" type="hidden"></input>
		    	<input id="wfRes" name="wfRes" value="${wfRes }" type="hidden"></input>
				<table class="tw-table-form tm-table-form">
					...
					<tr>
				   		<th>角色名称</th>
			    	 	<td>
			    	 		<input id="roleName"  name="roleName"  style="width: 260px;" value="${name }"></input>
			    	 	</td>
				    </tr>
				    <tr>
				   		<th>所属站点</th>
		    	 	...
		    	 	<tr>
		    	 		<th>绑定部门</th>
			    	 	<td>
			    	 		<textarea id="roleDepts"  name="roleDepts"  style="width: 560px;height:30px;" onclick="showDeptTree();">${deptNames }</textarea>
			    	 	</td>
		    	 	</tr>
			    	 <tr>
			    	 	<th>绑定人员</th>
			    	 	<td>
			    	 		<textarea id="roleEmps"  name="roleEmps"  style="width: 560px;height:30px;" onclick="showEmpTree();">${userNames }</textarea>
			    	 	</td>
			    	 </tr>
			    	 <tr>
			    	 	<th>门户权限</th>
			    	 	<td>
			    	 		<textarea id="ptlRight"  name="ptlRight"  style="width: 560px;height:30px;" onclick="showPortalRscList('1');">${portalResNames }</textarea>
			    	 	</td>
			    	 </tr>
			    	 <tr>
			    	 	<th>办公自动化</th>
			    	 	<td>
			    	 		<textarea id="oaRight"  name="oaRight"  style="width: 560px;height:30px;"  onclick="showPortalRscList('2');">${oaResNames }</textarea>
			    	 	</td>
			    	 </tr>
			    	 <%-- <tr>
			    	 	<th>流程</th>
			    	 	<td>
			    	 		<textarea id="wfRight"  name="wfRight"  style="width: 560px;height:30px;"  onclick="showPortalRscList('3');">${wfResNames }</textarea>
			    	 	</td>
			    	 </tr> --%>
			    	 <tr>
			    	 	<th>排序</th>
			    	 	<td>
			    	 		<input id="seq" name="seq" value="${seq }" style="width: 260px;"></input>
			    	 	</td>
			    	 </tr>
			    	  <tr>
			    	 	<th>默认</th>
			    	 	<td>
			    	 		<input id="ifDefault" name="ifDefault" type="checkbox" value="1" <c:if test="${ifDefault == 1}">checked="checked" </c:if> ></input> 
			    	 	</td>
			    	 </tr>
			    	 <tr>
...
<script type="text/javascript">
	function save(){
		...
		$.ajax({
			url : 'siteManage_updateRole.do',
			type : 'POST',   
			cache : false,
			async : false,
			data :$('#roleForm').serialize(),
			error : function(e) {  
				alert('AJAX调用错误(siteManage_updateRole.do)');
			},
