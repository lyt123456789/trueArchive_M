...
<body>
    <div class="wf-layout">
			<div class="wf-list-top" style="position: relative;">
				<div class="wf-search-bar">
					<form action="${ctx}/siteManage_showEmpForAuth.do" method="post" name="queryForm" id="queryForm">
			...
			<div class="">
				<table class="" layoutH="140" style="width: 100%;" cellspacing="0" id="allDiv">
					<tbody>
						<tr>
							<td style="width: 15%;" >
								<div class="wf-list-wrap" id="leftDiv" style="overflow:hidden"> 
											...
											<c:forEach var="item" items="${returnList}" varStatus="status">
												<c:choose>
													<c:when test="${item.issite eq '1' }">
														<tr onclick="changeVal('${item.id }')" id="${item.id }"><td title="${item.name }">${item.name }</td></tr>
													</c:when>
													<c:when test="${item.issite ne '1' && item.fatherDep eq item.siteId }">
													
														<tr id="${item.id }"><td onclick="showDep('${item.id}')" style="padding-left:45px;" title="${item.name }">${item.name }</td></tr>
													</c:when>
												</c:choose>
											</c:forEach>
						                </tbody>
							...
							<td style="width: 85%;">
								<iframe src="${ctx}/siteManage_showEmpForAuth.do?depId=${siteId}&name=${name }&begintime=${beginTime }&endtime=${endTime}"  id="rightDiv" scrolling="auto" frameborder="0" style="width: 100%;"></iframe>
							</td>
						</tr>
	                </tbody>
				</table>
	...
	$("#leftDiv").height($(window).height()-108+'px'); 
		...
		function doSearch(){
			...
			var myform = document.getElementById("queryForm");
	  		myform.action="${ctx}/siteManage_showAllEmpForAuth.do?username="+username+"&siteId="+sitename;
	  		myform.submit();
	  	}
				        ...
				        success : function(data){
				        	...
				        	if(success=='success'){
				        		 ...
				        		 $.each(json,function(j,item){
				        			 str +="<tr id='"+item.id+"' class='"+id+"tr' ><td onclick='showDep(\""+item.id+"\")' style='border-bottom: none !important;'><div style='margin-left: 65px;'>"+item.name+"</div></td></tr>";
				        		 });
				        		 tr.after(str);
				        	}
				        },  
	...
	function changeVal(id){
		$("#depId").val(id);
		//刷新rightDiv
		$('iframe').attr('src','${ctx}/siteManage_showEmpForAuth.do?depId='+id);  
	}
