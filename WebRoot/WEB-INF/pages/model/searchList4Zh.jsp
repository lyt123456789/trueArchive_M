...
<div class="search">
	<form name="list" onsubmit="return false;" id="list" action="${ctx }/model_toModelTask4Zh.do" method="post" style="display:inline-block;">
        	<input type="hidden" id="zzcdFlag" name="zzcdFlag" value="${zzcdFlag}"/><!-- 判断是否是临时用户 -->
        	<input type="hidden" id="searchType" name="searchType" value="${searchType}"/>
        	<input type="hidden" id="zhywIds" name="zhywIds" value="${zhywIds}"/>
		    ...
		    <span>查询范围：</span>
		    <select id="zhywType" name="zhywType">
				<option value="2" <c:if test="${'2' eq zhywType}">selected="selected"</c:if>>案卷目录</option>
				<option value="3" <c:if test="${'3' eq zhywType}">selected="selected"</c:if>>卷内目录</option>
			</select>
...
<script type="text/javascript">
	 window.onload=function(){ 
		//获得后台分页相关参数-必须
		skipUrl="<%=request.getContextPath()%>"+"/model_toModelTask4Zh.do?zhywIds=${zhywIds}";//提交的url,必须修改!!!*******************
...
<script type="text/javascript">
	function searchData(type){
		var zhywIds = parent.document.getElementById("zhywIds").value;
		...
		document.getElementById("zhywIds").value=zhywIds;
	...
	function addToDJK(){
		...
		$.ajax({
	        ...
	        cache:false,//是否使用缓存
	        dataType:"text",//返回值类型
	        data:{"djdId":djdId,"ids":ids,"structId":"${structId}","type":"2"},
