...
<html>
    ...
    <script type="text/javascript">
    	function saveToDb(){
    		if(document.getElementById('zwname').value.replace(/\s+/g,"")=="") {
        		alert("请填写文件名");
        		return;
    		}
    		saveFileToUrl('{ctx}/table_saveTemplateToDb.do','${name}');
    		if(confirm('是否关闭窗口?')){
    			 opener.location.href='<%=SystemParamConfigUtil.getParamValueByParam("contentpath")%>/table_getPendingList.do?itemid=<%=SystemParamConfigUtil.getParamValueByParam("dbItemId")%>';
        		closeWindow();
    		}
    	}
    </script>
    <body onload="openfile();">
    	<form action="${ctx}/template_upload.do" id="form1" name="form1" method="post">
	    	<div id="taoda" style="width: 100%;height: 540px;">
				<!-- 
	            <input type="text" name="name" id="name" value="${docvo.docVo.title}" size="60">
	            <select name="model" id="model" onchange="openfile(this.value)">
	                <option value="">请选择模板</option>
	                <c:forEach var="model" items="${modeList}">
	                    <option value="${model.localation }">${model.filename }</option>
	                </c:forEach>
	            </select>
	            --> 
	            ...
	            <c:if test="${isToDb=='1'}">
	            	 文件名<input type="text" name="zwname" id="zwname" style="width: 200px">&nbsp;
	            	<input type="button" value="保存至待办" onclick="saveToDb();">&nbsp;&nbsp;&nbsp;
	            </c:if>
   	...
   	<script type="text/javascript" src="${ctx}/widgets/plugin/js/base/jquery.js"></script>
   		<script type="text/javascript" src="${ctx}/contorl/ntko/tangerocx.js"></script>
    <script type="text/javascript">
    (
			function(){
			    //在页面呈现后,获取文档控件对象
