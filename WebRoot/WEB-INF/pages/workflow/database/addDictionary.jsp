...
<body>
    <div class="menu">
    	<form id="dictionaryform" action="${ctx}/dictionary_add.do" method="post" onsubmit="return checkForm();">
			<table class="infotan" width="100%">
				<tr>
 					<td width="15%" class="bgs ls">字典名称：</td>
 					<td width="85%">
						<input type="text" name="dictionary.vc_name" id="dictionary.vc_name" onblur="checkDictionaryName();" style="height: 22px;width:200px">
				...
				<tr>
					...
					<td>
						<input type="hidden" id="b_public" name="b_public">
			    	 	<input type="radio" name="b_public_rad" id="b_public_rad1" value="1" checked>公共字典
			    	 	<input type="radio" name="b_public_rad" id="b_public_rad0" value="0">私有字典
			    	 	<input type="hidden" name="dictionary.lcid" id="dictionary.lcid">
					</td>
				</tr>
			</table>
    			...
    			<thead>
		    	 ...
		    	 <tbody id="tbodyid">	
			    	 <tr valign="middle"  onclick="c1(this)">
		    	 		<td align="left" >
		    	 			<input type="text" name="vc_keys" id="vc_keys" style="height: 22px;width:380px">
		    	 		</td>
		    	 		<td align="left" >
		    	 			<input type="text" name="vc_values" id="vc_values" style="height: 22px;width:380px">
		    	 		</td>
		    	 		<td align="left" >
		    	 			<input type="radio" name="vc_default" id="vc_default" value="1" style="height: 22px;width:200px">
		    	 		</td>
		    	 		<td></td>
		    	 	</tr>
	    		</tbody>
    ...
    <script type="text/javascript">
	    function checkDictionaryName(){
			...
			if(vc_name!=""){
				$.post("${ctx}/dictionary_checkDictionary.do?vc_name="+vc_name, null, function(value) {
					 if(value=="yes"){
						  alert(' 此字典已存在!') ;
						  document.getElementById('dictionary.vc_name').value='' ;
						  document.getElementById('dictionary.vc_name').focus();
						  return false;
					 }
				 });
			}
	    ...
	    function checkForm(){
			...
			if(key.length > 1){
				document.getElementById('dictionary.vc_key').value = key.substring( 0, key.length - 1);
			}
			//字段名
			...
			if(value.length > 1){
				document.getElementById('dictionary.vc_value').value = value.substring( 0, value.length - 1);
			}
