...
<%@ include file="/common/headerindex.jsp"%>
	...
	<form action="${ctx}/itemRelation_editItemRelation.do" name="itemrelationform" id="itemrelationform" method="post">
		<input type="hidden" name="wfItemRelation.item_id" id="item_id" value="${wfItemRelation.item_id}">
		<input type="hidden" name="wfItemRelation.id" id="wfItemRelation.id" value="${wfItemRelation.id}">
	<table width="100%">
		<tr>
			...
			<td width="80%">
				<select id="delay_item_id" name="wfItemRelation.delay_item_id" onchange="getTables();" style="width: 300px;">
					...
					<c:forEach items="${list}" var="wfItem" varStatus="vs">
						<option value="${wfItem.id}" <c:if test="${wfItemRelation.delay_item_id eq wfItem.id}">selected</c:if> >${wfItem.vc_sxmc}</option>
					</c:forEach>
				</select>
			</td>
		</tr>
			...
			<td>
				<select id="table_id" name="wfItemRelation.table_id" onchange="getColoum();" style="width: 300px;">
					...
					<c:forEach items="${tablelist}" var="table" varStatus="vs">
						<option value="${table.id}" <c:if test="${wfItemRelation.table_id eq table.id}">selected</c:if>>${table.vc_tablename}[${table.vc_name}] </option>
					</c:forEach>
				</select>
			</td>
		</tr>
			...
			<td>
				<select id="coloum" name="wfItemRelation.coloum" style="width: 300px;">
					...
					<c:forEach items="${fieldlist}" var="field" varStatus="vs">
						<option value="${field.vc_fieldname}" <c:if test="${wfItemRelation.coloum eq field.vc_fieldname}">selected</c:if>>${field.vc_fieldname}[${field.vc_name}]</option>
					</c:forEach>
				</select>
			</td>
		</tr>
		<tr>
			...
			<td>
				<input id="reserve_value" name="wfItemRelation.reserve_value" type="text" style="width: 300px" value="${wfItemRelation.reserve_value}"/>
			</td>
		</tr>
		<tr>
			<td  class="bgs ls">延期天数：</td>
			<td>
				<input id="delay_date" name="wfItemRelation.delay_date" type="text" style="width: 100px" value="${wfItemRelation.delay_date}"
		...
		<%-- <tr>
			...
			<td>
				<input type="radio" name ="wfItemRelation.is_only" value="1" <c:if test="${wfItemRelation.is_only eq '1' }">checked</c:if>> 是
				<input type="radio" name ="wfItemRelation.is_only" value="2" <c:if test="${wfItemRelation.is_only eq '2' }">checked</c:if>> 否
			</td>
		</tr> --%>
	</table>
