	...
	<table style="height:100px;width:99%" border="0" align="center" cellpadding="0" cellspacing="0" class="contentTable">
       <!--  <tr>
        ...
        <tr>
          ...
          <div style="margin-top:12px;width:100px; float:left;height:35px">
            <SELECT  name="tableNames"  id="tableNames" style="width: 205px;height:22px;line-height:22px;font-size:12px">                 
            	<option value=""></option>
            	<c:forEach var="e" items="${tableNames}" varStatus="status"> 
                	<option value="${e.tableName}">${e.tableName}</option>
                </c:forEach>
			</SELECT>
			</div>
        ...
        <tr>
          ...
          <td width="50%"><input type="text" name="dicName"  id="dicName"  maxlength="50"  style="width:205px;"/></td>
		...
		<div style="width:100%;height:197px; overflow-y:scroll; overflow-x:hidden;">
			<table id="tableInfo" class="table_solid sjzdupdate_table_solid" border="0" cellspacing="0" >
			<tr>
			</tr>
		    </table>
		</div>
...
function selectTable(){
	...
	if(tableName==null||tableName==""){
		...
		$.ajax({   
	        url : '${ctx}/dataCenter_getTableInfo.do',
	        type : 'POST',   
	        cache : false,
	        async : false,
	        error : function() {  
	        ...
	        data : {
	            'tableName':tableName,'modId':modId
	        },    
	        success : function(data) {
	        	var res = eval("("+data+")");
	        	var html="";
	        	...
	        	if(tableInfoSize>0){
	        		for(var i=0;i<tableInfoSize;i++){
		        		 html=html + '<tr id="'+i+'" ><td style=" width:14%"><input style="width:90%;height:20px;line-height:20px;font-size:12px;" unselectable="on" type="text" id="filedName'+i+'" name="filedName'+i+'" value="'+res[i].filedName+'" readonly="true"/></td>'
		        				+'<td style=" width:10%"><input style="width:90%;height:20px;line-height:20px;font-size:12px;" unselectable="on" type="text" id="filedType'+i+'" name="filedType'+i+'" value="'+res[i].filedType+'" readonly="true"/></td>'
		        				+'<td style=" width:6%"><input style="width:90%;height:20px;line-height:20px;font-size:12px;" unselectable="on" type="text" id="filedLength'+i+'" name="filedLength'+i+'" value="'+res[i].filedLength+'" readonly="true"/></td>'
		        				+'<td style=" width:10%"><input style="width:90%;height:20px;line-height:20px;font-size:12px;" unselectable="on" type="text" id="filedDeafult'+i+'" name="filedDeafult'+i+'" value="'+(res[i].filedDeafult==null? "":res[i].filedDeafult)+'" readonly="true"/></td>'
		        				...
		        				+'<td style=" width:22%"><input style="width:90%;height:20px;line-height:20px;font-size:12px;" type="text" id="filedPk'+i+'" name="filedPk'+i+'" value="'+(res[i].filedPk==null? "":res[i].filedPk)+'"/></td></tr>';
		        	}
	        		$("#tableInfo tr:gt(0)").remove();
		        	$('#tableInfo tr').after(html);
		        	$('#tableInfo').after('<input type="hidden" id="tableInfoSize" name="tableInfoSize" value="'+tableInfoSize+'" />');	
		        	$(".dataDicaddlist:odd").css({"background-color":"#e0e0e0"});
			    	$(".dataDicaddlist:even").css({"background-color":"#f2f2f2"});
			    	tableNameDesc=res[0].tableNameDesc;
		        	}
	        	if(tableNameDesc==null){
	        		tableNameDesc="";
	        	}
	        	document.getElementById("dicName").innerText=tableNameDesc;
	        }
	    }); 
	}
}
