...
<body style="overflow: auto;" onload="initField();">
	<div class="menu">
    	<form id="templateform" action="${ctx}/template_editRed.do" method="post" enctype="multipart/form-data" onsubmit="return checkForm();">
			<table class="infotan" width="100%">
				<tr>
 					...
 					<td width="80%">
						...
						<input type="hidden" name="template.isRedTape" id="template.isRedTape" value="${template.isRedTape}">
						<input type="hidden" name="template.reflcId" id="template.reflcId" value="${template.reflcId}">
				...
				<tr>
					...
					<td width="80%"><span onclick="openNTKO('${template.vc_path}')" style="cursor: hand" class="uedword">编辑模板</span></td>
				</tr> 
					...
					<td width="20%" class="bgs ls">位置：</td>
				 	<td width="80%">
						<select name="template.position" id="template.position" style="width: 235px" onchange="control();">
				 			<option value="0" <c:if test="${template.position=='0'}">selected</c:if>>文档开头</option>
				            <option value="1" <c:if test="${template.position=='1'}">selected</c:if>>文档结尾</option>
						</select> 
				 	</td>
				</tr>
				<tr>
					<td width="20%" class="bgs ls">底部模板关联：</td>
				 	<td width="80%">
						<select name="relationId" id="relationId" <c:if test="${template.position=='1'}">disabled</c:if> style="width: 235px">
							<option value="" >请选择</option>
							<c:if test="${template.position=='0'}">
							<c:if test="${fn:length(relationTemplate) > 0}">
							<c:forEach items="${relationTemplate}" var="item" varStatus="status">
								<option value="${item.id }" <c:if test="${template.relationId == item.id}">selected</c:if> >${item.vc_cname }</option>
							</c:forEach>
							</c:if>
							</c:if>
						</select> 
