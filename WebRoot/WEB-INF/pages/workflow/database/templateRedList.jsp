...
<body>
    <div class="wf-layout">
		<div class="wf-list-top" style="height:62px;">
			<div class="wf-search-bar">
				<div class="wf-top-tool">
		            <a class="wf-btn" href="javascript:location.href='${ctx}/template_toAddJsp.do?isRedTape=${isRedTape}&lcid=${lcid}'" external="true" rel="CreatingForms" target="_self">
		                <i class="wf-icon-plus-circle"></i> 新建
		            </a>
		            <a class="wf-btn-green" onclick="javascript:refTemplate();" href="javascript:;">
		...
		<input type="hidden" id="isRedTape" name="isRedTape" value="${isRedTape }"/>
			    	...
			    	<tbody>
						<c:forEach items="${list}" var="d" varStatus="n">
					    	<tr>
						    	...
						    	<td align="center">
						    		<c:if test="${d.position == '0' }">
						    			头部
						    		</c:if>
						    		<c:if test="${d.position == '1' }">
	...
	function xg_row(){
		 ...
		 if(id){
		 	location.href = "${ctx}/template_toEditRedJsp.do?id="+id+"&isRedTape="+isRedTape;
		 }else{
			alert('请选择要修改的模板！');
	     }
	}
	...
	function refTemplate(){
		var isRedTape = document.getElementById('isRedTape').value;
		var url ="${ctx}/template_toRefJsp.do?isRedTape="+isRedTape+"&num="+Math.random();
		//用layer的模式打开
		layer.open({
			title:'选择其他流程模板',
			type:2,
			...
			yes:function(index,layero){
	            ...
	            iframeWin.ref();
			 	layer.close(index);
			 	window.location.href = "${ctx}/template_getTemplateList.do?isRedTape="+isRedTape+"&t="+new Date();
			}
		}); 
	}
		...
		function del(){
			...
			location.href = "${ctx}/template_del.do?ids="+ids+"&isRedTape="+isRedTape;
		}
	</script>
</html>
