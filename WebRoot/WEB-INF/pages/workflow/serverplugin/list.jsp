...
<body>
	<div class="wf-layout">
		<div class="wf-list-top" style="height:62px;">
			<div class="wf-search-bar">
				<div class="wf-top-tool">
		            <a class="wf-btn" href="${ctx}/serverPlugin_toAdd.do"  target="_self">
		...
		<form id="thisForm" method="POST" name="thisForm" action="${ctx }/serverPlugin_getList.do" >
	...
	<script type="text/javascript">
		window.onload=function(){ 
			//获得后台分页相关参数-必须
			skipUrl="<%=request.getContextPath()%>"+"/serverPlugin_getList.do";			//提交的url,必须修改!!!*******************
	...
	function xg_row(){
		 ...
		 if(id){
		 	location.href = "${ctx}/serverPlugin_toUpdate.do?id="+id+"&type=${type }";
		 }else{
			alert('请选择要修改的组！');
	     }
	}
	...
	function ck_row(){
		 var id=$(".wf-actived td:eq(2)").attr("beanid");
		 if(id){
		 	location.href = "${ctx}/serverPlugin_view.do?id="+id+"&type=${type }";
		 }else{
			alert('请选择要查看的组！');
	     }
	}
	...
	function deleteSelected(checkboxesName){
		...
		g.g('ids').value=ids;
		g.g('thisForm').action='${ctx}/serverPlugin_delete.do';
		g.g('thisForm').submit();
	};
	</script>
</body>
