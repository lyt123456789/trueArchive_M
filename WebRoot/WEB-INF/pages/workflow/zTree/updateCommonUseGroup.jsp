		    ...
		    <div class="tw-usertree-group">
				...
				<div class="tw-bd">
					<table width="100%" class="tw-table-form">
						<tr>
						 	<th width="30%">组名：</th>
						 	<td width="68%"><input type="text" name="groupName" id="groupName" value="${groupName }" size="40"/></td>
						</tr> 
						<tr>
						 	<th width="30%">排序号：</th>
						 	<td width="68%"><input type="text" name="orderIndex" id="orderIndex" value="${orderIndex }" size="40"/><font color="red">（注：按数值大小倒序）</font></td>
						</tr>
						<tr>
						 	<th style="width:30%;">是否机要组：</th>
						 	<td style="width:68%;"><input type="checkbox" name="isJyGroup" id="isJyGroup" <c:if test="${isJyGroup == '1' }">checked="checked"</c:if> /><font color="red">（注：创建机要组请勾选）</font></td>
						</tr>
					</table>				
				</div>
			</div>
