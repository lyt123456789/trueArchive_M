...
<!--页面独立树状图生成及操作函数-->
<script src="${ctx}/widgets/zTree/assets/js/function.zTree.dapt.js?t=20190311" type="text/javascript" charset="utf-8"></script>
<%-- <script src="${ctx}/widgets/zTree/assets/js/function.zTree.js?t=20180705" type="text/javascript" charset="utf-8"></script> --%>
<!-- 引入layer  -->
...
.JS_treeulist.JS_scrollDt{
	cursor: pointer;
	background: #fff;
	border:1px solid #bdbdbd!important;
}
.JS_scrollD .tw-usertree-list {
	cursor: pointer;
	background: #fff;
	border:1px solid #bdbdbd!important;
}
.JS_scrollD.active .tw-usertree-list{
	border:2px solid rgb(55, 167, 255)!important;
}
.overComment{
	width: 600px;
...
var dbDapts = '${dpUserIds}';
var isHasValue = '${isHasValue}';
_globalObject.isShowZr = '${isShowZr}';
$(document).ready(function() {
	if(isHasValue && isHasValue=='1'){
		$('#JS_scrollPer').addClass("active");
		$('#JS_scrollDapt').removeClass("active");
	}else{
		$('#JS_scrollPer').removeClass("active");
		$('#JS_scrollDapt').addClass("active");
	}
	if(dbDapts) {
		_globalObject.selectedPartPerDapt = dbDapts.split(',');
	}
	var jsonurl = '';
	if(null != status && status=='all'&&isDuBan==''){
		jsonurl = '${ctx}/widgets/zTree/ajaxData/getTreeGroup4All.json?t=' + new Date().getTime();
	}else if(null != status && status=='all'&& isDuBan=='yes'){
			...
			if(data.length>0){
				...
				var laterLoad = function(){
					...
					$.each(data, function(i,e){
						...
						_url = addUrlPara(_url,'isDuBan',isDuBan);
...
<div class="tw-usertree-choice clearfix" style="height: auto;background-position: 70.5% 40%;">
    ...
    <!-- <input type="hidden" id="dbUserIds" value=""/> -->
	...
	<div class="tw-usertree-select" style="width: 25%;">
		<div class="tw-mod">
			<div class="tw-hd" style="font-size:16px;">已经选择 <span>(<b id="countNum">0</b>)</span><span class="fr tw-act" onclick="delAll();emptyText();">清除全部</span></div>
			<div class="tw-bd">
				<c:if test="${routType != '2' and empty isDuBan}">
					<div class="JS_wrap">
						<ul class="tw-usertree-list JS_hover active JS_treeulist JS_scrollt" data-keyName='A'></ul>
						<div class="clearfix"></div>
					</div>
				</c:if>
				<c:if test="${routType != '2' and not empty isDuBan}">
					<div class="JS_wrap">
						<ul class="tw-usertree-list JS_treeulist JS_scrollDt active" data-keyName='A'></ul>
						<div class="clearfix"></div>
					</div>
				</c:if>
...
<!-- 				<c:forEach var="m" items="${mapList}">
	...
	<c:if test="${routType != '2' and not empty isDuBan}">
		<div class="clearfix"></div>
		<div class="clearfix" style="margin-top: 6px;padding: 5px 20px">
			<div style="float: left;padding-top: 6px;">请</div>
			<div style="float: left;width: 30%;margin: 0 2%;" id="JS_scrollPer" class="JS_scrollD active">
				<textarea class="tw-usertree-list JS_scrollDp" id="lingdao" readonly="readonly" style="padding-left: 4px;padding-top: 3px;" ></textarea>
			</div>
			<div style="float: left;width: 15%;">
				<input type="text" id="yueshi" style="height: 35px;width: 100%;padding:8px;border: 1px solid #ccc;" value="${empty yueshiText?'阅示':yueshiText}"  />
			</div>
			<div style="float: left;width: 30%;margin: 0 2%;" class="JS_scrollD" id="JS_scrollDapt">
				<textarea class="tw-usertree-list JS_scrollDb" id="chushi" readonly="readonly" style="padding-left: 4px;padding-top: 3px;" ></textarea>
			</div>
			<div style="float: left;width: 15%;">
				<input type="text" id="banli" style="height: 35px;width: 100%;padding:8px;border: 1px solid #ccc;" value="${empty banliText?'阅处':banliText}" />
			</div>
		</div>
	</c:if>
</div>
...
<div class="tw-search-bar tar">
	<div class="overComment">
		<textarea id="commentInfo" style="height: 60px;width: 100%;padding:8px;border: 1px solid #ccc;"  >${jsondata}</textarea>
	</div>
	<div class="tw-top-tool" style="font-size:14px;">
		<c:if test="${showCheckbox != null && showCheckbox != '' && showCheckbox eq '1' }">
			...
			<div class="piaochecked on_check" style="float:left;margin-top:-2px;width: 20px;height:20px;"><input id="sendAllMsg" style="float:left" name="need_inv" type="checkbox" style="height:20px;width:20px;" class="radioclass input" value="1" title="发短信" onclick="onSelectAllChange(event)" style="margin-top:-2px"></div> <span style="    display: inline-block; margin-top: 2px;">全部短信通知</span>
...
<!-- <div class="tw-btn-primary " style="cursor:pointer;font-size:14px;" onclick="test();">确认选择</div> -->
	...
	function cdv_getvalues(){
		...
		if(document.getElementById("sendMsg") && document.getElementById("sendMsg").checked){
			...
			if(top.window && top.window.process && top.window.process.type){
            	...
            	var selectedPartPerDapts = _globalObject.selectedPartPerDapt.join(',');
            	//console.log(selectedPartPerDapts);
            	if(stringNotEmpty(commentVal)){
           		...
           		if(null != sendMsg && '' != sendMsg){
            		return $("#ret").val()+'$$'+ selectedPartPerDapts+'$$'+$('#yueshi').val()+'$$'+ $('#banli').val()+"#"+sendMsg+commentVal;
            	}else{
            		return $("#ret").val()+'$$'+ selectedPartPerDapts+'$$'+$('#yueshi').val()+'$$'+ $('#banli').val()+commentVal;
            	}
			}
	}
...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#lingdao,#chushi,#yueshi,#banli").focus(function(){
			clearInterval(timer);
			openTimer();
		});
	});
	function openTimer(){
		timer = setInterval(function() {
			var text = "";
			var lingdaoText = $('#lingdao').val();
			var chushiText = $('#chushi').val();
			if(stringNotEmpty(lingdaoText)){
				text += "请"+lingdaoText+$('#yueshi').val()+"，";
			}
			if(stringNotEmpty(chushiText)){
				text += "请"+chushiText+$('#banli').val()+"，";
			}
	...
	function stringNotEmpty(str){
		if(str==null||str==''||str=='undefined'||myTrim(str)==''){
			return false;
		}
		return true;
	}
	...
	function emptyText(){
		...
		$('#yueshi').val('阅示');
		$('#banli').val('阅处');
		_globalObject.selectedSubPrincipal = [];
		_globalObject.selectedPrincipal = [];
