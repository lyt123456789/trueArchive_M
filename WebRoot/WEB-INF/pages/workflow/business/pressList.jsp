...
<div class="wf-layout">
	<div class="wf-list-top">
		<div class="wf-search-bar">
			<form name="pressList"  id="pressList" action="${ctx }/business_getPressList.do" method="post">
	...
	<div class="wf-list-wrap">
		<form id="thisForm" method="POST" name="thisForm" action="${ctx }/business_getPressList.do" >
			<table class="wf-fixtable" layoutH="140" >
		    	...
		    	<c:forEach items="${list}" var="item" varStatus="status">
			    	<tr>
						...
						<td  align="center">
							<c:choose>
								<c:when test="${item.pressStatus != '0'}">
									<span>
										已催办
									</span>
								</c:when>
								<c:otherwise>
									<c:choose>
										<c:when test="${isAdmin==1 }">
											...
											onclick="pressMsg('${item.wf_process_uid}');" 
											title="催办">
								                	催办
								            </button>
										</c:when>
	...
	<script type="text/javascript">
		window.onload=function(){ 
			//获得后台分页相关参数-必须
			skipUrl="<%=request.getContextPath()%>"+"/business_getPressList.do";			//提交的url,必须修改!!!*******************
		...
		function exportexcel(){
			document.forms.pressList.action="${ctx}/business_exportPressList.do";
			document.forms.pressList.submit();
		}


		//短信编辑弹出窗口
		function pressMsg(processId){
			var time = new Date();
			var url = '${ctx}/business_getPressMsg.do?processId='+processId+'&time='+time;
			layer.open({
	            title :'催办信息',
	            content: url,
	            type: 2, 
