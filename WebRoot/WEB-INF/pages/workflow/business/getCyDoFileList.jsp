			...
			<table class="wf-fixtable" layoutH="140">
		    	...
		    	<c:forEach items="${doFileList}" var="doFile" varStatus="status">
					...
					<td style="text-align: left; padding-left: 10px;" title="${doFile.doFile_title}"><span style="width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
					 <a href="#" onclick="openForm('${doFile.processId}','${doFile.itemId}','${doFile.formId}','${favourite}','${doFile.doFile_title}','${doFile.instanceId}');">
						<c:choose>  
...
<script type="text/javascript">
	...
	function openForm(processId,itemId,formId,favourite,process_title,instanceId){
		$.ajax({
			url : 'business_updateHandRoundShip.do',
			type : 'POST',
			cache : false,
			async : false,
			error : function() {  
				alert('AJAX调用错误(business_updateHandRoundShip.do)');
			},
			data : {
