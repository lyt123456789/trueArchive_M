...
<div class="wf-layout">
	<div class="wf-list-top">
		<div class="wf-search-bar">
			<form name="pressList"  id="pressList" action="${ctx }/business_getStatisticalList.do" method="post">
 	        <input type="hidden" name="isAdmin"  value="${isAdmin}" />
	            ...
	            <button class="wf-btn-green" onclick="exportexcel();">
	                	导出
	            </button>
            </form>
		</div>
	...
	<div class="wf-list-wrap">
		<form id="thisForm" method="POST" name="thisForm" action="${ctx }/business_getStatisticalList.do" >
			<table class="wf-fixtable" layoutH="140">
				<thead>
		    	  	<tr>
						...
						<th width="120px">办理时间</th>
						<th width="100px">事项类别</th>
						<th width="100px">是否办理</th>
                    </tr>
		    	</thead>
		    	<c:forEach items="${list}" var="item" varStatus="status">
			    	<tr>
						...
						<td align="center">
							<c:if test="${item.isover == 'NOT_OVER'}">
								<span class="wf-badge-doing">未办</span>
							</c:if>
							<c:if test="${item.isover == 'OVER'}">
								<span class="wf-badge-over">已办</span>
							</c:if>
						</td>
					</tr>	
		    	</c:forEach>
	...
	<script type="text/javascript">
		window.onload=function(){ 
			//获得后台分页相关参数-必须
			skipUrl="<%=request.getContextPath()%>"+"/business_getStatisticalList.do";			//提交的url,必须修改!!!*******************
		...
		function search(){
			document.forms.pressList.action="${ctx}/business_getStatisticalList.do";
			document.forms.pressList.submit();
		}
		
		function exportexcel(){
			document.forms.pressList.action="${ctx}/business_exportStatisticalList.do";
			document.forms.pressList.submit();
			//document.forms.pressList.action="${ctx}/business_getPressList.do";
		}
