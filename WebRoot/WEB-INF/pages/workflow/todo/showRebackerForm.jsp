    ...
    <c:if test="${status=='1'}">
       <a href="javascript:void(0);"  class="accessory ml5" onclick="changeClass()" >附件<em id="fjCount"></em></a>
	        <div class="bl_n_nav bl_a_nav fouce" style="display: none;z-index:2;width: 810px; height : 100px;overflow: auto;" id="fjml">
	          <div class="bl_nav_fj" >
	         	 <span id="tmshow"></span>
    			<trueway:att onlineEditAble="false" id='${receiveId}tm' docguid='${receiveId}tm' showId="tmshow" ismain="true" downloadAble="true" uploadAble="false" deleteAble="false" previewAble="true" tocebAble="false" toStampAble="false" openBtnClass="icon-add" otherBtnsClass="icon-help" uploadCallback="loadCss" deleteCallback="loadCss"/>
      		</div>
      	</div>
    </c:if>
    <c:if test="${status=='2'}">
    ...
    <c:if test="${status=='1'}">
       <div class="nr fl ml10">
    	 ...
    	 <c:if test="${receiveType == '0'}">
    	  <div class="fl ml8 dxdhz">
    	 <a style="cursor: pointer;;margin-left: 30px;color: white;" href="#" onclick="pushDb();"><span>协办反馈</span></a> 
    	  </div></c:if>
	  <!--  	  <div class="fl ml8 dxdhz">
		     <span onclick="downUrl();"  style="cursor: pointer;margin-left: 30px;color: white;" >下载</span>
		     </div>
			--> 
		
		     <div class="fl ml8 dxdhz">
		     <span onclick="downWord()" style="cursor: pointer;margin-left: 15px;color: white;"  >原文下载</span>
		     </div> 
		     <div class="fl ml8 dxdhz">
		     <span onclick="tm();"  style="cursor: pointer;margin-left: 30px;color: white;" >脱密</span>
		     </div>
		     <div class="fl ml8 dxdhz">
		     <span onclick="print(0);"  style="cursor: pointer;margin-left: 15px;color: white;" >脱密打印</span>
		     </div>
		       <div class="fl ml8 dxdhz">
		     <span onclick="print(1)" style="cursor: pointer;margin-left: 10px;color: white;"  >红章打印</span>
		     </div> 
		   <div class="fl ml8 sydyfs">剩余打印份数:<span id="sydyfs" style="color: red;"></span></div>
		 </div>	 
		 </div>  
      </c:if>
     <c:if test="${status=='0'}">
     <div class="nr fl ml10">
    	 <div class="fl zc_box">
	    	 <div class="fl ml8 dxdhz">
		     	<span onclick="updateStatus();"  style="cursor: pointer;margin-left: 30px;color: white;" >收取</span>
		     </div>
		 </div>	 
		 </div>  
     </c:if>
...
$(document).ready(function() {
	$.ajax({  
		url : 'table_getDyfs.do?receiveId=${receiveId}',
		type : 'POST',   
		cache : false,
		async : false,
		error : function() {  
		...
		success : function(msg) {
			if(document.getElementById("sydyfs")!=null){
				document.getElementById("sydyfs").innerHTML=msg;
				fs=msg;
			}
    ...
    $('#li_dir').hover(over, out);
	   ...
	   if("${!((isFirst == true || isCy == true || (stepIndex=='1' && cType !='1')) && (isWriteNewValue != false) && (finstanceId==null || finstanceId=='' ))}"!='true'){
	    ...
	    if(  document.getElementById('fjml')){
	    	  ...
	    	  var tmshow = document.getElementById("tmshow").innerHTML;
	    	  if(tmshow!=null && tmshow!=''){
...
function PrintPDF(){
	 document.getElementById("frame1").contentWindow.PrintPDF();
}
//<script>
...
function PrintNoHdr_ToDb(nodeId, workFlowId){    
    var operate = 0;
	var obj = document.getElementById("frame1").contentWindow;
	var tagValue = getProValue();//得到属性值   json object
	var instanceId = eval(obj.document.getElementById("instanceId")).value;
...
function showTrueJson(){
	...
	function goExChangeSendVlaue(){
		...
		$.ajax({  
			...
			//url : 'table_getMaxProcessIdByAllInstanceId.do?allInstanceId='+allInstanceId,
			type : 'POST',   
			cache : false,
			async : false,
			data :{
...
function print(type){
	...
	var dyfs=obj.printTrue(fs,downUrl,type);
	if(type==1){
		if(dyfs>fs){
			dyfs=fs;
		}
		document.getElementById("sydyfs").innerHTML=fs-dyfs;
		fs=fs-dyfs;
		$.ajax({  
