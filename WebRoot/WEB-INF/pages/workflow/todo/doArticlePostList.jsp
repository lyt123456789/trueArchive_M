	...
	<div class="wf-layout">
		...
		<div class="wf-list-top">
			<div class="wf-search-bar">
	 	       		...
	 	       		<input type="text" class="wf-form-text" name="bt" value="${bt }" placeholder="输入关键字"/>
	                ...
	                <label class="wf-form-label" for="">来文时间：</label>
				...
				<table class="wf-fixtable" layoutH="140">
					<thead>
			    	  	<tr>
							...
							<th width="15%" align="center">来文号</th>
							<th width="15%" align="center">来文单位</th>
							<th width="15%" align="center">来文时间</th>
			    	...
			    	<c:forEach var="item" items="${list}" varStatus="status"> 
						<tr>
							<td align="center" width="5%" >${(selectIndex-1)*pageSize+status.count}</td>
							<td align="center" width="20%" style="text-align: left;" title="${item[0]}">
								<c:if test="${('1' eq item[11])}">
									<c:if test="${!('1' eq item[10])}"><span class="wf-badge-main">主</span></c:if>
					   			 	<c:if test="${('1' eq item[10])}"><span class="wf-badge-sub">子</span></c:if>
				   			 	</c:if>
								<a href="#" onclick="openForm('${item[7]}','${item[8]}','${item[9]}', '${item[0]}');">
								<c:choose>  
							...
							<td align="center" width="15%" title="${item[1]}">${item[1]}</td>
							<td align="center" width="15%" title="${item[2]}">${item[2]}</td>
							<td align="center" width="15%" title="<fmt:formatDate value="${item[3]}" pattern="yyyy-MM-dd"></fmt:formatDate>"><fmt:formatDate value="${item[3]}" pattern="yyyy-MM-dd"></fmt:formatDate></td>
							
							<c:if test="${'1' eq isFiling }">
							<td align="center" width="5%" height=36>
							<c:if test="${null == item[5] }">
							<a href="javascript:void(0)" onclick="filedDofile('${item[7]}','${item[8]}','${item[9]}')" class="icon icon-guidang" title="归档" >
