...
<head>
	...
	<link rel="stylesheet" href="${ctx}/qrcode/iconfont.css" type="text/css" />
...
<div class="wf-layout">
	<div class="wf-list-top">
		<div class="wf-search-bar" style="position: relative;">
		  	<form name="doItemList"  id="doItemList" action="${ctx }/table_getDoFileList.do?laterSearch=1" method="post" style="display:inline-block;">
				...
				<input type="hidden" name="isFgw" value="${isFgw}"/>
				<input type="hidden" name="siteId" value="${siteId}"/>
				<input type="hidden" name="pdfNameToWh" value="${pdfNameToWh}"/>
				<input type="hidden" name="isShowJjcd" value="${isShowJjcd}"/>
				<input type="hidden" name="jjcd" value="${jjcd}"/>
				<input type="hidden" name="excelSpecial" value="${excelSpecial}" id="excelSpecial"/>
			    ...
			    <div class="wf-top-tool">
					...
					<!-- <a class="wf-btn-primary" onclick="javascript:downloadExpBatch();" target="_self">
						Excel导出
					</a> -->
                ...
                <input type="text" class="wf-form-text w-auto-10" id="wfTitle" name="wfTitle" style="width: 75px" value="${wfTitle}" placeholder="输入关键字">
		  	...
		  	<form name="doItemListH"  id="doItemListH" action="${ctx }/table_getDoFileList.do?laterSearch=1" method="post" style="display:none;">
				<input type="hidden" name="isPortal" value="${isPortal}"/>
				<input type="hidden" name="laterSearch" value="${laterSearch}"/>
				<input type="hidden" name="favourite" value="${favourite}"/>
				<input type="hidden" name="isShowExp" value="${isShowExp}"/>
			...
			<div id="high-search" style="height:<c:choose><c:when test="${not empty isShowJjcd and isShowJjcd =='1'}">388px</c:when><c:otherwise>355px</c:otherwise></c:choose>;background-size: cover;">
				<form method="post" id="form" name="form" action="#" class="tw-form">
            		...
            		<table class="new-htable">
		                ...
		                <c:if test="${not empty isShowJjcd and isShowJjcd =='1'}">
			                <tr>
			                    <th>紧急程度：</th>
			                    <td>
			                    <select class="tw-form-select" name="jjcd" id="jjcd">
					                	<option value=""></option>
						 				<option value="特急" <c:if test="${jjcd eq '特急'}">selected="selected"</c:if>>特急</option>
						 				<option value="急" <c:if test="${jjcd eq '急'}">selected="selected"</c:if>>急</option>
						 				<option value="一般" <c:if test="${jjcd eq '一般'}">selected="selected"</c:if>>一般</option>
					              </select></td>
			                </tr>
		                </c:if>
		                <tr>
		                    		...
		                    		<c:when test="${isFgw =='1'}">
			                    		<tr>
					                    	...
					                    	<select class="tw-form-select" name="departId" id="departId">
							                	<option value=""></option>
							                	<c:forEach items="${fgwDepts}" var="dept">
	...
	<div class="wf-list-wrap">
			...
			<table class="wf-fixtable" layoutH="140">
		    	...
		    	<c:forEach items="${doFileList}" var="doFile" varStatus="status">
			    	<tr onclick="dianji('${doFile.doFile_id}')">
			    		<c:if test="${favourite == '1' || isShowExp == '1'}">
						<td align="center">
    						<input type="checkbox" name="selid" value="${doFile.doFile_id}" onclick="changeCheck();" >
    					</td>
    					</c:if>
						...
						<td style="text-align: left;" title="${fn:contains(doFile.doFile_title,'*')&&fn:indexOf(doFile.doFile_title,'*')==36?fn:substring(doFile.doFile_title,37,-1):doFile.doFile_title}">
							<c:choose>
							...
							<c:choose>
								<c:when test="${'3' eq doFile.urgency}">
									<img src="${ctx}/images/jinji_tj.png">
								</c:when>
								<c:when test="${'2' eq doFile.urgency}">
							...
							<c:if test="${doFile.isCanRead eq '1' || isShowExp == '1'}">
								<a href="#" onclick="openForm('${doFile.processId}','${doFile.itemId}','${doFile.formId}','${favourite}','${fn:contains(doFile.doFile_title,'*')&&fn:indexOf(doFile.doFile_title,'*')==36?fn:substring(doFile.doFile_title,37,-1):doFile.doFile_title}');">
									 <c:choose>  
										<c:when test="${fn:length(fn:contains(doFile.doFile_title,'*')&&fn:indexOf(doFile.doFile_title,'*')==36?fn:substring(doFile.doFile_title,37,-1):doFile.doFile_title) > 24}">  
	   										<c:out value="${fn:substring(fn:contains(doFile.doFile_title,'*')&&fn:indexOf(doFile.doFile_title,'*')==36?fn:substring(doFile.doFile_title,37,-1):doFile.doFile_title, 0, 24)}..." />  
										</c:when>  
										<c:otherwise>  
							...
							<c:if test="${doFile.isCanRead != '1' && isShowExp != '1' }">
						...
						<td align="center" title="${doFile.entrustName}">${doFile.entrustName }</td>
						<c:choose>
							<c:when test="${isShowExp == '1' && isFgw == '1'}">
								<td align="center">${fn:substring(doFile.applyTime, 0, 16)}</td>
							</c:when>
							<c:otherwise>
								<td align="center">${fn:substring(doFile.applyTime, 0, 16)}</td>
							</c:otherwise>
						...
						<c:if test="${isShowWH != '1' }">
						  <td align="center">
							...
							<c:if test="${doFile.doFile_result==2&&fgw_fw_item eq doFile.itemId}">
								<i class="iconfont icon-erweima qrcodeSpan" onclick="getQrCode('${doFile.instanceId}')"></i>
							</c:if>
						</td>
						</c:if>
						</c:if>
		...
		<div id="qrcodePic" onclick="hideDiv()">
		</div>
		<img id="qrcodePic-img" alt="" src="" onclick="hideDiv();">
		<c:if test="${fn:length(doFileList)==0  && laterSearch == '1'}">
...
<script type="text/javascript">
		window.onload=function(){ 
			//获得后台分页相关参数-必须
			skipUrl="<%=request.getContextPath()%>"+"/table_getDoFileList.do";			//提交的url,必须修改!!!*******************
...
<script type="text/javascript">
	...
	function exportEx() {
		...
		var excelSpecial = document.getElementById("excelSpecial");
		if(excelSpecial){
			excelSpecial = excelSpecial.value;
		}
		var isFgw='${isFgw}';
		...
		if ($('#loadMask').css('display') === 'block') {
			$.ajax({  
				...
				+"&commitTimeTo="+commitTimeTo+"&wh="+encodeURI(encodeURI(wh))+"&lwdw="+encodeURI(encodeURI(lwdw))+"&isFavourite="+isFav+"&departId="+departId+"&excelSpecial="+excelSpecial+"&isFgw="+isFgw,
				type : 'POST',   
				cache : false,
				async : true,
				error : function() {  
				...
				success : function(msg) {
					var downUrl = "${ctx}/tableExtend_downloadForExcel.do?fileNameWithPath="+encodeURI(encodeURI(msg))+"&excelSpecial="+excelSpecial;
					if(obj){
						obj.src=downUrl;
					}else{
	...
	function downloadBatch(){
		...
		if(ids){
			if(confirm("如果有加密附件，仅导出表单页")){
				var obj = document.getElementById("download_iframe");
				var excelSpecial = document.getElementById("excelSpecial");
				if(excelSpecial){
					excelSpecial = excelSpecial.value;
				}
				var downUrl = "${ctx}/table_downloadBatch.do?ids="+ids+"&excelSpecial="+excelSpecial;
				if(obj){
					obj.src=downUrl;
				}else{
					window.location.href=downUrl;
	...
	function getQrCode(instanceId){
		 $.ajax({
			 type : 'post',
			 url : '${ctx}/table_getQrcodeUrl.do',
			 data : {
				 'instanceId':instanceId
			 },
			 success : function(data){
				 $('#qrcodePic').show();
				 $('#qrcodePic-img').attr('src',data);
				 $('#qrcodePic-img').show();
			 },
			 error : function(){
				 alert('AJAX调用错误(table_getQrcode.do)');
			 }
		 });
	}
	function hideDiv(){
		 $('#qrcodePic').hide();
		 $('#qrcodePic-img').attr('src','');
		 $('#qrcodePic-img').hide();
	}
	
	function downloadExpBatch(){
		var selid = document.getElementsByName('selid');
		var ids = "";
		for(var i = 0 ; i < selid.length; i++){
			if(selid[i].checked){
		...
		if(ids){
			var obj = document.getElementById("download_iframe");
			var downUrl = "${ctx}/table_downloadExpBatch.do";
			$.ajax({
				type :'post',
				url : downUrl,
				data : {
				...
				success:function(msg){
					...
					var downUrl2 = "${ctx}/tableExtend_downloadForExcel.do?fileNameWithPath="+encodeURI(encodeURI(msg));
					if(obj){
						obj.src=downUrl2;
					}else{
						window.location.href=downUrl2;
					}
				},
				error:function(){
					alert("AJAX调用错误(tableExtend_downloadForExcel.do)");
				}
			});
		}else{
			alert("请选择需要导出的办件");
...
/*$(window).on('load', function () {
	...
	if (!flag) {
		$('#thisForm').after('<div style="padding-top:100px;font-size:18px;text-align:center">很抱歉，没有找到'+ '<span style="padding-left:10px;padding-right:18px;color:red">' + msg + '</span></div>')
	}
})*/
</script>
</html>