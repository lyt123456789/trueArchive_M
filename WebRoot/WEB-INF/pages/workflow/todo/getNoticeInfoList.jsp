...
<div class="wf-layout">
	<div class="wf-list-top">
		<div class="wf-search-bar">
			<form name="pendingList" id="pendingList" action="${ctx }/noticeInfo_getNoticeInfoList.do" method="post">
		...
		<form id="thisForm" method="POST" name="thisForm" action="${ctx}/comment_getCommentList.do" >
			<table class="wf-fixtable" layoutH="140">
		    	...
		    	<c:forEach var="item" items="${list}" varStatus="status"> 
					...
					<td>
						<a href="#" onclick="openReplyForm('','${item.fprocessId}');">${item.title}</a>
					</td>
					<td>${item.userName}</td>
					<td>
...
<script type="text/javascript">
	...
	window.onload=function(){ 
			//获得后台分页相关参数-必须
			skipUrl="<%=request.getContextPath()%>"+"/noticeInfo_getNoticeInfoList.do";			//提交的url,必须修改!!!*******************
