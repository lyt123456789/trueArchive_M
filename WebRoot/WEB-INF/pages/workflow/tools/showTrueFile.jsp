...
<div class="bl_nav">
	...
	<ul class="clearfix">
	      <li id="li_dir" style="padding-bottom:  10px;">
	      	...
	      	<iframe height="203px" id="mliframe" width="99px"  frameborder=no  style="display:none;orphans:0 ;float:left;padding:0px;margin:0px;position:absolute;left:0px;top:42px;z-index:111;" src="${ctx}/ml.jsp?imageCount=${imageCount}"></iframe>
	     </li>     
	      <li>
		...
		$('.bl_tab').each(function(i){
	    	$(this).click(function(){
	    		if(i==0){
	    			...
	    			$('#frame2').height(0);
	    			//alert($("#frame3"));
	    			//$("#frame3").css("z-index","0").css('display','none');
	    			//$('#frame3').css("z-index","0").css('display','none');
	    			...
	    			//$('#frame1').css("z-index","9").css('display','block');
	    			$('#frame2').height(0);
	    			//$('#frame2').css("z-index","0").css('display','none');
	    			if('${imageCount}'>1){
				...
				$.ajax({   
					...
					success : function(person) {  
						...
						if(isMergeChild == '1'){
							...
							window.close();
												window.returnValue='refresh';					//window.location.href="${ctx}/table_getPendingList.do";
											}else{
												window.close();
												window.returnValue='refresh';
											}
									...
									success : function(msg) {
										...
										if(res.result =='1'){							//未选择部门时,直接办结办件,但是徐要给出提醒
											...
											if('${isFirst}' == 'true'){
												...
												window.returnValue='refresh';
											if('${isPortal}'=='1'){
window.close();												window.returnValue='refresh';												//window.location.href="${ctx}/table_getPendingList.do";
												}
											}
