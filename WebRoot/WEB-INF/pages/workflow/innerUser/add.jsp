...
<body > 
	<form id="form1" method="POST" name="thisForm" action="${ctx }/form_addForm.do">
			...
			<input type="hidden" name="isSystemGroup" value="${isSystemGroup }"/>
			<table class="infotan" width="100%">
				<tr>
 					<td width="20%" class="bgs ls">组名：</td>
 					<td width="80%"><input type="text" name="innerUser.name" id="teamName" size="40"/></td>
				</tr> 
				<tr>
 					<td class="bgs ls">排序号：</td>
 					<td><input type="text" name="innerUser.zindex" id="sortId" size="40"/><font color="red">（注：按数值大小倒序）</font></td>
				</tr>
				<tr>
 					<td class="bgs ls">组优先级：</td>
 					<td><input type="text" name="innerUser.grade" id="grade" size="40"/>（注：非必填按需设置）</td>
				</tr>
				<tr>
	...
	<script type="text/javascript">
			...
			function sub(){
				...
				if(teamName == ""){
				    ...
				    if(!reg.test(sortId)||(grade != "" &&!reg.test(grade))){
				        alert("请输入数字!");
				        return ;
					}else{
						$.ajax({
			                ...
			                cache: false,
			                async: false,
			                data:"id="+id+"&teamName="+teamName+"&isSystemGroup=${isSystemGroup}"+"&type=4&add="+true+"&lcid=${lcid}",
			   			    error: function(){
			   			        alert('AJAX调用错误');
			   			    },
			   			    success: function(msg){
			   			        if(msg=='yes'){
									...
									alert('数据保存中,请耐心等待。。。');
									document.getElementById('form1').action='${ctx}/group_addInnerUser.do?isSystemGroup=${isSystemGroup}&lcid=${lcid}';
									document.getElementById('form1').submit();
									//window.location.href="${ctx}/group_getInnerUserList.do?type=4";
			   			        }
			   			    }
			   			});
					}
			...
			function goHistroy(){
				...
				window.location.href="${ctx}/group_getInnerUserList.do?id=${lcid}"
			}	   
		</script>
