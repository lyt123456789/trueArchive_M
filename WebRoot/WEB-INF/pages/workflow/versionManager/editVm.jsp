...
<body>
<form id="vmform" method="post" action="${ctx}/vm_editVm.do?id=${id}" style="font-family: 宋体; font-size: 12pt;">
<table class="infotan" width="100%">
	<tr>
		<td width="20%" class="bgs ls">版本号：</td>
		<c:if test="${isAdmin == 1}">
			<td width="80%"><input type="text" name="num" id="num" style="width: 200px;height: 22px" value="${vm.num}"/></td>
		</c:if>
		<c:if test="${isAdmin != 1}">
			<td width="80%"><span>${vm.num}</span></td>
		</c:if>
	</tr>
	<tr>
		...
		<td width="80%">
			<c:if test="${isAdmin == 1}">
				<textarea name="content" cols="100" rows="20" id="content">${vm.content}</textarea> 
			</c:if>
			<c:if test="${isAdmin != 1}">
				<span>${vm.content}</span>
			</c:if>
		</td>
	</tr>
	<tr>
		...
		<td>
			<c:if test="${isAdmin == 1}">
				<input type="text" name="applyProject" id="applyProject" style="width: 200px;height: 22px" value="${vm.applyProject}">
			</c:if>
			<c:if test="${isAdmin != 1}">
				<span>${vm.applyProject}</span>
			</c:if>
		</td>
	</tr>
	<tr>
	  ...
	  <td width="80%">
		  ...
		  <c:if test="${isAdmin != 1}">
     		 <trueway:att onlineEditAble="false" id="${instanceId}fj" docguid="${instanceId}fj" showId="fjshow" ismain="false" downloadAble="true" uploadAble="false" deleteAble="false" previewAble="true" tocebAble="false" toStampAble="true" openBtnClass="icon-add" otherBtnsClass="icon-help" uploadCallback="loadCss" deleteCallback="loadCss"/>
		  </c:if>
	 </td>
  </tr>
</table>
