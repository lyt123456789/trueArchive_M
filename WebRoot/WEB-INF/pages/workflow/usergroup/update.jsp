...
<head>
	...
	<body>
		...
		<form id="form1" method="POST" name="thisForm" action="${ctx }/form_addForm.do">
			...
			<input type="hidden" name="userGroup.id" value="${userGroup.id }" id="teamId"/>
		<table class="infotan" width="100%">
		<tr>
		 <td width="20%" class="bgs ls">组名：</td>
		 <td width="80%"><input type="text" name="userGroup.name" id="teamName" value="${userGroup.name }"/></td>
		</tr> 
		<tr>
		 <td class="bgs ls">排序号：</td>
		 <td><input type="text" name="userGroup.zindex" id="sortId" value="${userGroup.zindex}"/>（注：按数值大小倒序）</td>
		</tr>
		<tr>
			 <td class="bgs ls">组优先级：</td>
			 <td><input type="text" name="userGroup.grade" id="grade" value="${userGroup.grade}"/>（注：非必填按需设置）</td>
		</tr>
		</table>
				...
				<td style="">
						...
						<select id="oldSelect" size="20" style="width:100%;height: 290px;border: 1px dashed #C2C2C2" multiple="multiple">
							<c:forEach var="m" items="${userGroup.employeeList}">
								<option value="${m.employeeGuid }|${m.employeeName }|${m.department.departmentShortdn }">${m.employeeName} {${m.department.departmentShortdn}}</option>
							</c:forEach>
						</select>
				</td>
			</tr>
			...
			function sub(){
				...
				if(teamName == ""){
				    ...
				    if (!re.test(sortId)||(grade != "" &&!re.test(grade))){
						...
						alert('数据保存中,请耐心等待。。。');
						g.g('form1').action='${ctx }/userGroup_updateUserGroup.do';
						g.g('form1').submit();
					}
				}
			};
