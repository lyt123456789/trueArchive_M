...
$(document).ready(function() {
	...
	$.ajax({
		...
		//接口数据
		url: '${ctx}/zTree/ajaxData/getTreeGroupSet.json?t=' + new Date().getTime(),
		dataType: "json",
		async:false,
	...
	<body>
    <div class="tw-usertree-wrap clearfix" style="height: 100%;padding-bottom:0px;padding-top:0px;">
	    ...
	    <div class="tw-usertree-right">
				...
				<div class="tw-usertree-select" >
				    <div class="tw-mod" style="margin-bottom: 0px;">
...
<!-- 	<div class="tw-search-bar tar">
	...
	$(document).ready(function() {
		//加载layer皮肤
	  	...
	  	layer.config({
	     	 	extend: ['skin/layer-tw.css'], //加载新皮肤
	...
	function deleteGroup(gid){
		layer.confirm("确定要删除吗？",function(){
			$.ajax({
		        ...
		        dataType:"text",//返回值类型
		        data:{"gid":gid},
		        url:"${ctx}/departmentTree_deleteUserGroup.do",
				...
				yes:function(index,layero){
					...
					if(name!=''){
						$.ajax({
							...
							url:"${ctx}/departmentTree_modifyUserGroup.do",
							success:function(text){
								if(text=='ok'){
									layer.close(index);
			...
			yes:function(index,layero){
				...
				if(name!=''){
					$.ajax({
						...
						dataType:"text",//返回值类型
						data:{"name":name},
						url:"${ctx}/departmentTree_addUserGroup.do",
						success:function(text){
	...
	function showUsers(o){
		...
		$.ajax({
			...
			url:"${ctx}/departmentTree_getGroupUsers.do",
			success:function(text){
				if(text){
					var ret = eval("("+text+")");
		...
		if(groupId){
			...
			//}else{
				...
				$.ajax({
					...
					cache:false,//是否使用缓存
					dataType:"text",//返回值类型
					data:{"persons":JSON.stringify(persons),"groupId":groupId},
					url:"${ctx}/departmentTree_addGroupUsers.do",
					success:function(text){
						if(text=='ok'){
							layer.msg("更新人员组人员成功",{time:1000,icon:1});
						}else{
							layer.msg("更新人员组人员失败",{time:1000,icon:0});
						}
					}
				});
			//}
		}else{
			layer.msg("请选择人员组",{time:1000,icon:0});
		}
	}
