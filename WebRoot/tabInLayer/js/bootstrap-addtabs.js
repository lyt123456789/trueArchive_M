 ...
 * Version : 1.0
 *
 * Created by WHorse on 2016-7-20.
 */
...
window.Addtabs = {
    ...
    add: function (opts) {
        ...
        if (!$("#" + id)[0]) {
            //创建新TAB的title
            if($('li[role = "presentation"]').length < 5){
            var title = $('<li>', {
                'role': 'presentation',
                'id': 'tab_' + id
            }).append(
            ...
            obj.children(".tab-content").append(content);
                ...
                $('div[role = "tabpanel"]:last').addClass('active');			
			}
        }
        ...
        $("#" + id).addClass("active");
		$('.tab-content>.tab-pane>.iframeClass').css('z-index','-10');
		$('.tab-content>.tab-pane>.iframeClass').css('z-index','-999999px');
		$('.tab-content>.active>.iframeClass').css('left','0px');
		$('.tab-content>.active>.iframeClass').css('z-index','10');
        Addtabs.drop();
    },
    close: function (id) {
        //如果关闭的是当前激活的TAB，激活他的前一个TAB
        if (obj.find("li.active").attr('id') == "tab_" + id) {
			if($("#tab_" + id).prev('li[role = "presentation"]').length ==1 ){
