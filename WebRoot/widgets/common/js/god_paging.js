function Paging(maxindex,pagesize,selectIndex){
			...
			this.currentPagePosInMax=5;//当前页面在分页条数中的位置
			...
			if(this.pageCount<=this.maxCountPerPage){
				...
				this.startpage=1;
				this.endpage=this.startpage+7;
			}else if(this.selectIndex>this.currentPagePosInMax){
				this.startpage=this.selectIndex-this.currentPagePosInMax+1;
				if(this.startpage+7<=this.pageCount){
					this.endpage=this.startpage+7;
				}else{
					this.endpage=this.pageCount;
			...
			this.lastPage=function(){
				...
				url=this.skipUrl+'?pageIndex='+((first-2)*this.pageSize)+'&selectIndex='+(first-1);
			...
			this.nextPage=function(){
				this.setQueryParm();
				first=parseInt(this.selectIndex,10);
				url=this.skipUrl+'?pageIndex='+(first*this.pageSize)+'&selectIndex='+(first-1+2);
				this.formObj.action=url;
				this.formObj.submit();
			}	
			//首页跳转
			...
			this.endPage=function(){
				this.setQueryParm();
				url=this.skipUrl+'?pageIndex='+((this.pageCount-1)*this.pageSize)+'&selectIndex='+this.pageCount;
				this.formObj.action=url;
				this.formObj.submit();
			}
			//用户填数字跳转
			this.goPage=function(objid){
				...
				url=this.skipUrl+'?pageIndex='+((goCount-1)*this.pageSize)+'&selectIndex='+goCount;
				this.formObj.action=url;
				this.formObj.submit();
				return true;
			}
			...
			this.selectPage=function(selectIndex){
				this.setQueryParm();
				url=this.skipUrl+'?pageIndex='+((selectIndex-1)*this.pageSize)+'&selectIndex='+selectIndex;
				this.formObj.action=url;
				this.formObj.submit();
			}
			...
			this.setState=function(overStyle,outStyle,styleType){
				...
				if(this.selectIndex==1){
					for(var i=0;i<2;i++){
						var obj=document.getElementById(ids[i]);
						if(obj!=null){
							document.getElementById('first_sp_page_img').src='widgets/common/js/first_sp_page_img2.png';
							document.getElementById('last_sp_page_img').src='widgets/common/js/last_sp_page_img2.png';
							obj.onclick=null;
							obj.style.cursor='';
				...
				if(this.selectIndex==this.pageCount||this.pageCount==1){
					for(var i=2;i<4;i++){
						var obj=document.getElementById(ids[i]);
						if(obj!=null){
							document.getElementById('next_sp_page_img').src='widgets/common/js/next_sp_page_img2.png';
							document.getElementById('end_sp_page_img').src='widgets/common/js/end_sp_page_img2.png';
			...
			this.sortBySql=function(columnname){
				if(this.sortObj!=null){					
					...
					url=this.skipUrl+'?pageIndex='+((this.selectIndex-1)*this.pageSize)+'&selectIndex='+this.selectIndex;
					this.formObj.action=url;
					this.formObj.submit();
				}else{
...
function startPaging(MaxIndex,pageSize,selectIndex,sortStr,skipUrl,submitForm,innerObj){
	...
	var allInfo='找到&nbsp;&nbsp;'+paging.MaxIndex+'&nbsp;&nbsp;个记录，显示&nbsp;&nbsp;'+paging.startCount+'&nbsp;&nbsp;至&nbsp;&nbsp;'+paging.endCount+'&nbsp;&nbsp;个信息';
	
	//设置页数选择链接-可选可删可修改-onclick事件不能改变
	var selectStr="";
	for(var j=paging.startpage;j<=paging.endpage;j++){
		var selStyle='margin-right:0px;cursor:pointer;';
		//if(j==selectIndex)selStyle='margin-right:0px;cursor:pointer;background-color:#00FFFF';
		if(j==selectIndex)selStyle='background:url(widgets/common/js/xz.png) no-repeat center;height:25px;padding-top:13px;display:inline-block';
		selectStr+="<span style='"+selStyle+"' onmouseover=\"paging.changeStyle(this,'color','red')display:inline\" onmouseout=\"paging.changeStyle(this,'color','')\" onclick='paging.selectPage("+j+")'>"+"&nbsp;&nbsp;"+j+"&nbsp;&nbsp;"+"</span>";
	}
	...
	var startlast="<span class='ml5' id='first_sp_page' onclick='paging.startPage()' class='fy_menu' style='cursor: pointer;'><img id='first_sp_page_img' src='widgets/common/js/first_sp_page_img1.png' /></span>&nbsp;&nbsp;<span>首页</span>&nbsp;&nbsp;&nbsp;&nbsp;" +
			"<span class='ml5' id='last_sp_page' onclick='paging.lastPage()' class='fy_menu' style='cursor: pointer;'><img id='last_sp_page_img' src='widgets/common/js/last_sp_page_img1.png' /></span><span class='ml5'>上页</span><span class='ml5'></span>";

	//设置下一页末页链接-可选可删可修改-onclick事件、id名不能改变
	var endnext="<span class='ml5'>下页</span><span class='ml5' id='next_sp_page' onclick='paging.nextPage()' class='fy_menu' style='cursor: pointer;'><img id='next_sp_page_img' src='widgets/common/js/next_sp_page_img1.png' /></span>&nbsp;&nbsp;&nbsp;&nbsp;" +
			"<span class='ml5'>末页</span><span class='ml5' id='end_sp_page' onclick='paging.endPage()' class='fy_menu' style='cursor: pointer;'><img id='end_sp_page_img' src='widgets/common/js/end_sp_page_img1.png' /></span>";

	//设置GO链接-可选可删可修改-onclick事件、id名不能改变
	var go="<span class='ml5'>共计"+paging.pageCount+"页，跳转到</span><input class='ml5' id='go_t' type='text' style='width: 30px;height: 15px;border: 1px solid #999999;padding-bottom:2px;padding-top:3px;'/>&nbsp;&nbsp;<span style='cursor: pointer;'><img src='widgets/common/js/pageGo.png'  onclick=\"return paging.goPage('go_t')\"/></span>";
	...
	var pageStr='<table   style="width:100%;border:0px;">'
					+'<tr>'
						...
						+startlast+'&nbsp;'+selectStr+'&nbsp;'+endnext+'&nbsp;'+'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+go
						+'</td>'
					+'</tr>'
				+'</table>';
	if(innerObj){
