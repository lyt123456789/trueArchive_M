function Paging(maxindex,pagesize,selectIndex){
			...
			this.maxCountPerPage=5;//一页显示的分页条数
			this.currentPagePosInMax=3;//当前页面在分页条数中的位置
			...
			if(this.pageCount<=this.maxCountPerPage){
				...
				if(this.startpage+4<=this.pageCount){
					this.endpage=this.startpage+4;
				}else{
					this.endpage=this.pageCount;
			...
			this.lastPage=function(){
				...
				url=this.skipUrl+'?pageIndex='+((first-2)*this.pageSize)+'&selectIndex='+(first-1)+'&pageSize='+this.pageSize;
			...
			this.nextPage=function(){
				this.setQueryParm();
				first=parseInt(this.selectIndex,10);
				url=this.skipUrl+'?pageIndex='+(first*this.pageSize)+'&selectIndex='+(first-1+2)+'&pageSize='+this.pageSize;
				this.formObj.action=url;
				this.formObj.submit();
			}	
			//首页跳转
			...
			this.endPage=function(){
				this.setQueryParm();
				url=this.skipUrl+'?pageIndex='+((this.pageCount-1)*this.pageSize)+'&selectIndex='+this.pageCount+'&pageSize='+this.pageSize;
				this.formObj.action=url;
				this.formObj.submit();
			}
			//用户填数字跳转
			this.goPage=function(objid){
				...
				url=this.skipUrl+'?pageIndex='+((goCount-1)*this.pageSize)+'&selectIndex='+goCount+'&pageSize='+this.pageSize;
				this.formObj.action=url;
				this.formObj.submit();
				return true;
			}
			...
			this.changePagesize=function(pageSize){
				this.setQueryParm();
				url=this.skipUrl+'?pageSize='+pageSize+'&pageIndex=0';
				this.formObj.action=url;
				this.formObj.submit();
				return true;
			}
			...
			this.selectPage=function(selectIndex){
				this.setQueryParm();
				url=this.skipUrl+'?pageIndex='+((selectIndex-1)*this.pageSize)+'&selectIndex='+selectIndex+'&pageSize='+this.pageSize;
				this.formObj.action=url;
				this.formObj.submit();
			}
