...
var Class = {
    ...
    CDrag.load = function (obj_string, func, time,params) {
    //加载对象 20110902修改
    	var index = 0, timer = window.setInterval(function () {
    		try {
    			if (eval(obj_string + ".loaded")) {
    				window.clearInterval(timer);
    				func(eval("new " + obj_string+'("'+params+'")'));
    			}
    		} catch (exp) {}
    ...
    CDrag.Table.Rows.prototype = {
    	...
    	initialize : function (id, parent, element, window, locks) {
			...
			//生成ICO
			CDrag.database.icoCurNum=(CDrag.database.icoCurNum%CDrag.option.icos);
			CDrag.database.icoCurNum+=1;
			bgico="ico"+CDrag.database.icoCurNum;
			wc.ico.className="tipico "+bgico;
    		//内容
    		wc.content = wc.element.childNodes[1].childNodes[0];
    		wc.content.setAttribute('id','text_'+wc.root_id);
    	...
    	element_init : function () {
    		var wc = this, div = _$("tempbox").cloneNode(true);
    	...
    	load : function () {
			...
			/**
			* 2011-09-02修改，优化异步加载 info.src为加载条件
			*/
    			//script = document.createElement("script");
    			//script.src = info.src + ".js"//, script.defer = true;
    			//document.getElementsByTagName("head")[0].appendChild(script);
    			CDrag.load(info.className, wc.upload.bind(wc), 5, info.src);//go to line_88
    ...
    CDrag.Add.prototype = {
		...
		wc.button2.onclick =wc.close.bind(wc);
    		wc.init_element();		
    	},
    ...
    CDrag.prototype = {
    	...
    	execMove : function (status, on_obj, in_obj, place) {
    		...
    		var maxw=_$('col-mid').offsetWidth;
    	...
    	create_json : function () {
    		...
    		if(window['isbtnsubmitok']){
    			...
    			ajax.post(CDrag.option.url,CDrag.option.paramName+'='+poststr.join('|'),function(){
    				return false;
    			});
				//alert(poststr);
    	...
    	parse : function (o) {
    		...
    		try {
    			...
    			for (i = 0 ; i < o.length ; i ++) {
    				div = _$(o[i].cols), cols = table.add(div);
