		...
		return $(this).each(function(){
			...
			var _defaults={
					...
					option:{
						...
						lc:'{',
						rc:'}',
						rowtpl:'<div id="{id}" class="jQrow {className}"><div class="title"><h2><span>{title}</span></h2><div class="tools"><span class="window"></span><span class="lock"></span><span class="close"></span></div></div><div class="content">{src}</div></div>',				//栏目模版
			...
			function bindDrag(){
				...
				$('.title',$(this)).hover(
					function(){
						$(this).find('.tools').stop().animate({top:'15px'},200);
					},function(){
						$(this).find('.tools').stop().animate({top:'-30px'},200);
					}
				);
