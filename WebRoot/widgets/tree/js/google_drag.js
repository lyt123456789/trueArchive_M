...
function after_Drag(){
	...
	this.elm.className='drag_div';
	
	//强制刷新iframe_清除缓存
	var childs=this.elm.childNodes;
	var further_child=childs[1].childNodes;
	for(var i=0;i<further_child.length;i++){
		if(further_child[i].tagName=='iframe'||further_child[i].tagName=='IFRAME'){
			var refresh_src=further_child[i].src;
			further_child[i].contentWindow.location=refresh_src;
		}	
	}
	// 在虚线框的地方插入拖动的这个元素
	...
	var uuu=document.getElementById('elementTable').offsetHeight;	
     parent.document.getElementById('element').style.height=uuu;
	return returnValue;
}
