...
(function(start,obj,tag,objw){
	...
	for(var i=0,len=navmenu.length;i<len;i++){
		navmenu[i].onmouseover=function(){
			...
			size.from=from;
			size.to=getAbsObjPoint(this)[0]-(Math.floor((objw/2)) - Math.floor((this.offsetWidth/2)));
	...
	var goback=function (){
		...
		from=isNaN(from)?0:from;
		size.from=from;
		size.to=getAbsObjPoint(navmenu[start-1])[0]-(Math.floor((objw/2)) - Math.floor((navmenu[start-1].offsetWidth/2)));
		if(Math.abs(size.to-size.from)<1) return false;
		//缓动效果
		movebg(_$(obj),size,0.1,5);
	};
