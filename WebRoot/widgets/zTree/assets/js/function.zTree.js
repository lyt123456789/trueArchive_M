...
//		})
		$.each(list, function(i,e){
			...
			var inputSpan = '<div class="piaochecked on_check"><input name="need_inv" type="checkbox" style="height:20px;width:20px;" class="radioclass input" value="1" title="发短信" onchange="onInputCheck()"></div>'
...
function toggleSelectOption(id,name){
	...
	//if($(".tw-usertree-select").find("[data-id='"+id+"']").length<1){
		var _html = '<li data-pId=' + _globalObject.pIdList[id] + ' data-id='+ id +'><span>'+ name +'</span><i class="tw-usertree-del tw-icon-times-circle" onclick="deltree(this)"></i><div class="piaochecked on_check"><input name="need_inv" type="checkbox" style="height:20px;width:20px;" class="radioclass input" value="1" title="发短信" onchange="onInputCheck()" /></div></li>';
...
function createTabsHtml(data){
	...
	_globalObject.initScrollPos = function() {
		...
		if(_globalObject.isMobiles){
			...
			var pw = $('#pleaseOrWatch');
			if(pw && pw.val()){
				leftZTreeHeight = leftZTreeHeight +40;
				rightAreaHeight = rightAreaHeight +35;
			}
