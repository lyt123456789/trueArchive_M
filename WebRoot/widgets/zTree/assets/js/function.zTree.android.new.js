...
_globalObject.selectedTreeIdNums = [];
					...
					$.each(ssNodes, function(index2, item2) {
						...
						if(sssNodes && sssNodes.length > 0) {
							$.each(sssNodes, function(index3, item3) {
								...
								if(!subsNodes) {
									$("#" + item3.parentTId).addClass('no-childs');
								}
							})
						} else {
							$("#" + item2.parentTId).addClass('no-childs');
						}
					})
				} else {
					$("#" + item1.parentTId).addClass('no-childs');
				}
			})
		}
	})
	$('#' + treeId).find('.switch').hide();
}
...
//		url: apiUrl+"?"+sid+"="+encodeURI(encodeURI(str))+"&t=" + new Date().getTime(),
		...
		turl = addUrlPara(turl,'userId', $("#userId").val());
		$.ajax({
...
function dataRelation(treeId){
	...
	if(changeList){
		if( typeof(changeList)=="object" || changeList.length<1){
			...
			if(_globalObject.isMobiles) {
				for(var i=0,l=_globalObject.selectedList.length;i<l;i++){
					changeList.push(_globalObject.selectedList[i]);
				}
			} else {
	...
	$.each(_globalObject.selectedList, function(i,e) {
		...
		if(treeNodes.length>0){
			$.each(treeNodes, function(j,f) {
				zTree.checkNode(f, true, true, true);
			});
		}
	});
	// 全选已选人数优化
	...
	for(var key in _globalObject.idList) {
		if(key == treeId) {
			if(count == _globalObject.idList[key].length) {
				$('#' + treeId).parents('.tw-panel-cnt').next('.tw-bottom-bar').find('#checkAll').addClass('on');
			} else {
				$('#' + treeId).parents('.tw-panel-cnt').next('.tw-bottom-bar').find('#checkAll').removeClass('on');
			}
		}
	}
	$('#' + treeId).parents('.tw-panel-cnt').next('.tw-bottom-bar').find('#num').text(count);
...
//				var nodes = zTree.getNodesByParam('type','file',treeNode);
	...
	if(_globalObject.isMobiles) {
		...
		//var nodeIdList = _globalObject.idList;
		//var nodeIdListLength = 0;
		//for(var key in nodeIdList) {
		//	if(treeId == key) {
		//		nodeIdListLength = nodeIdList[key].length;
		//	}
		//}
		//if(nodeIdListLength == _globalObject.selectedList.length) {
		//	$("#" + treeId).parents('.tw-panel-cnt').next('.tw-bottom-bar').find("#checkAll").addClass('on');
		//} else {
		//	$("#" + treeId).parents('.tw-panel-cnt').next('.tw-bottom-bar').find("#checkAll").removeClass('on');
		//}
		//console.log(_globalObject.selectedList)
		//$("#" + treeId).parents('.tw-panel-cnt').next('.tw-bottom-bar').find("#num").text(_globalObject.selectedList.length)
	}
...
function toggleSelectOption(id,name){
	...
	//if($(".tw-usertree-select").find("[data-id='"+id+"']").length<1){
		var _html = '<li data-id='+ id +'><span>'+ name +'</span> <i class="tw-usertree-del tw-icon-times-circle" onclick="deltree(this)"></i><input style="float:right;margin-top:4px;margin-right:5px" title="发短信" type="checkbox"/></li>';
...
function createTabsHtml(data){
	...
	$.each(data, function(i,e){
		//组成头部标签
		html += '<li class="tw-nav-item" data-domId="' + e.domId + '"><a href="javascript: void(0)">'+e.name+'</a></li>';
		//组成搜索框，绑定时器
		...
		body += '<div class="tw-tab-panel">'
	            	+'<div class="tw-panel-cnt">'
	            		...
	            		+'</div>'
	            	+'</div>'
					+'<div class="tw-bottom-bar clearfix"><a class="" id="checkAll"><i class="icon-radio1"></i><span>全选</span></a><span class="people-nums">已选择<span class="color-blue" id="num">0</span>人</span></div>'
	            +'</div>';
	...
	_globalObject.initScrollPos = function() {
		...
		var navHeight = $(".tw-tabs-nav").height();
		var bottomHeight = $(".tw-bottom-bar").height();
		tmpHeight = tmpHeight<100?100:tmpHeight;
		//tmpHeight = tmpHeight>400?400:tmpHeight;
		if(isAndroid) {
			tmpHeight = tmpHeight - navHeight - bottomHeight;
		}
		if(isiOS) {
			tmpHeight = tmpHeight - navHeight - bottomHeight - 10;
		}
		
		$('.JS_scroll').css({'height':tmpHeight+'px','overflow-x':'auto','overflow-y':'auto'});
	}
	_globalObject.pageSize = getPageSize();
	var tmpHeight =  _globalObject.pageSize[3];
	var navHeight = $(".tw-tabs-nav").height();
	...
	if(_globalObject.isMobiles){
		...
		if($('.JS_treeulist').length>1){
			...
			$('.JS_scrollt').slimScroll({height: '357px'});
		}
