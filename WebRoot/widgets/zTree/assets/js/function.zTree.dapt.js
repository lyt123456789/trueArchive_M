...
_globalObject.selectedPartPer = []; //主任
...
function newDaptDom(treeId, zTree, node) {
	if (node.type == 'file') {
		if ($('#JS_scrollPer').hasClass('active')) {
			if(node.checked){
		...
		if ($('#JS_scrollDapt').hasClass('active')) {
			if(node.checked){
	...
	if (node.type == 'folder') {
		if ($('#JS_scrollPer').hasClass('active')) {
			...
			var pers = [];
			var principalPers = [];// 局长
			var subPrincipalPers = []; //副局长
			var dapts = [];
			$.each(nodes, function(i,e) {
				...
				if(e.isPrincipal == 'true') {
					principalPers.push(e.id + '_' + e.empsort);
				} else if(e.isPrincipal == 'false') {
					subPrincipalPers.push(e.id + '_' + e.empsort);
				}
			...
			if(node.checked){
				...
				if (_globalObject.isShowZr == '2') {
					_globalObject.selectedPrincipal = $.union(_globalObject.selectedPrincipal, principalPers);
					_globalObject.selectedSubPrincipal = $.union(_globalObject.selectedSubPrincipal, subPrincipalPers);
				}
			}else{
				if (_globalObject.isShowZr == '1') {
				...
				if (_globalObject.isShowZr == '2') {
					_globalObject.selectedPrincipal = $.except(_globalObject.selectedPrincipal, principalPers);
					_globalObject.selectedSubPrincipal = $.except(_globalObject.selectedSubPrincipal, subPrincipalPers);
				}
	...
	if(perNames.length >= 0 && PrincipalIds.length==0 && subPrincipalIds.length==0){
		$('.JS_scrollDp').val(perNames.join('、'));
	}else{
		if(PrincipalIds.length>0 && subPrincipalIds.length>0){
			$('.JS_scrollDp').val(PrincipalNames.join('、') + "、" + subPrincipalNames.join('、'));
		}else{
			if(PrincipalIds.length>0){
				$('.JS_scrollDp').val(PrincipalNames.join('、'));
			}
			if(subPrincipalIds.length>0){
				$('.JS_scrollDp').val(subPrincipalNames.join('、'));
			}
		}
	}
	...
	$('.JS_scrollDb').val(daptNames.join('、'));
...
function delIdCheck(id, pId, sort){
	...
	_globalObject.selectedSubPrincipal = $.except(_globalObject.selectedSubPrincipal, [id + '_' + sort]);
	_globalObject.selectedPrincipal = $.except(_globalObject.selectedPrincipal, [id + '_' + sort]);
}
