...
function createTabsHtml(data){
	...
	if(!_globalObject.isMobiles){
		...
		html += '<li class="tw-nav-act"><span class="tw-btn-green tw-btn-sm" onclick="selectAll()">选择全部</span></li>';
	...
	if(_globalObject.isMobiles){
		...
		if($('.JS_treeulist').length>1){
			...
			$('.JS_scrollt').slimScroll({height: '270px'});
		}
...
function selectAll(){
	...
	ztree.checkAllNodes(true);
	$.each(ztree.getNodes(), function(i,e) {
	  nodeOnCheck(null,'allOrg',e);
	});
	superEndDomBetter();
}