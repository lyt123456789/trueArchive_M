...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<link href="${ctx}/dwz/style/css/ued.demo.css" rel="stylesheet" type="text/css" />
		...
		<script type="text/javascript">
	        function initTree(){
		    	 $("#black").treeview({
				 	url: "departmentTree_getContentByPsfk.do?timestamp="+ new Date().getTime()+"&mc="
			     });
				var va = '${employees}'.split("&");
				var vb = '${employeeName}'.split("&");
				if(va&&va!=""){
					for(var i=0;i<va.length;i++){
						oldSelect.options.add(new Option(vb[i],va[i])); 
					}
				}
	        }
	        function sousuo(){
	        	...
	        	$("#black").treeview({
	        		url: "departmentTree_getContentByPsfk.do?timestamp="+ new Date().getTime()+"&mc="+encodeURI(encodeURI(name))
				 });
	        }
