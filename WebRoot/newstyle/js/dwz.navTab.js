...
var navTab = {
	...
	_op: {id:"layout", stTabBox:".navTab-tab",node:".node", stPanelBox:".navTab-panel", mainTabId:"main", close$:"a.close", prevClass:"tabsLeft", nextClass:"tabsRight", stMore:".tabsMore", stMoreLi:"ul.tabsMoreList"},
	...
	init: function(options){
		...
		this._leftnode=this.componentBox.find(this._op.node);
		...
		for(var i=0;i<this._leftnode.length;i++){
			this._nodecontextmenu(this._leftnode.eq(i));
			}
	...
	_nodecontextmenu:function($obj){ // navTab右键菜单
		var $this = this;
		var fl="";
		switch($obj.attr("fl"))
		{
		...
		$obj.contextMenu(fl, {
			bindings:{
				...
				nodeLook:function(t,m){
					var workflowid = $('#workflowname').attr("workflowid");
					//tabId[结点ID]
					//workflowid[工作流ID]
					//查看工作流程图
					//console.log(workflowid);
				},
				nodeUpdate:function(t,m){
					//修改工作流程图
				},
				nodeCreation:function(t,m){
					//生成工作流程图
				},
				nodeSubmit:function(t,m){
					//提交工作流
				}
			}
		});
	...
	openTab: function(tabid, url, options){ //if found tabid replace tab, else create a new tab.
		...
		//alert("obj:"+$(window.parent.document.body).find("#"+this._op.id).html());       
